...
Adom configuration gives you fine-grained control over the model, execution environment, and integrations available to the CLI. Use this guide alongside the workflows in [`adom exec`](./exec.md), the guardrails in [Sandbox & approvals](./sandbox.md), and project guidance from [AGENTS.md discovery](./agents_md.md).
...
Adom supports several mechanisms for setting config values:
...
- A generic `-c`/`--config` flag that takes a `key=value` pair, such as `--config model="o3"`.
  ...
  - For consistency with `config.toml`, values are a string in TOML format rather than JSON format, so use `key='{a = 1, b = 2}'` rather than `key='{"a": 1, "b": 2}'`.
    - The quotes around the value are necessary, as without them your shell would split the config argument on spaces, resulting in `adom` receiving `-c key={a` with (invalid) additional arguments `=`, `1,`, `b`, `=`, `2}`.
...
- The `$ADOM_HOME/config.toml` configuration file where the `ADOM_HOME` environment value defaults to `~/.adom`. (Note `ADOM_HOME` will also be where logs and other Adom-related information are stored.)

Both the `--config` flag and the `config.toml` file support the following options:

## Feature flags

Optional and experimental capabilities are toggled via the `[features]` table in `$ADOM_HOME/config.toml`. If you see a deprecation notice mentioning a legacy key (for example `experimental_use_exec_command_tool`), move the setting into `[features]` or pass `--enable <feature>`.
...
The model that Adom should use.

```toml
model = "gpt-5"  # overrides the default ("gpt-5-adom" on macOS/Linux, "gpt-5" on Windows)
```

### model_providers

This option lets you add to the default set of model providers bundled with Adom. The map key becomes the value you use with `model_provider` to select the provider.
...
# Name of the provider that will be displayed in the Adom UI.
name = "OpenAI using Chat Completions"
# The path `/chat/completions` will be amended to this URL to make the POST
# request for the chat completions.
base_url = "https://api.openai.com/v1"
# If `env_key` is set, identifies an environment variable that must be set when
# using Adom with this provider. The value of the environment variable must be
...
Note this makes it possible to use Adom CLI with non-OpenAI models, so long as they use a wire API that is compatible with the OpenAI chat completions API. For example, you could define the following provider to use Adom CLI with Ollama running locally:
...
Export your key before launching Adom: `export AZURE_OPENAI_API_KEY=…`
...
How many times Adom will retry a failed HTTP request to the model provider. Defaults to `4`.

##### stream_max_retries

Number of times Adom will attempt to reconnect when a streaming response is interrupted. Defaults to `5`.

##### stream_idle_timeout_ms

How long Adom will wait for activity on a streaming response before treating the connection as lost. Defaults to `300_000` (5 minutes).
...
If the selected model is known to support reasoning (for example: `o3`, `o4-mini`, `adom-*`, `gpt-5`, `gpt-5-adom`), reasoning is enabled by default when using the Responses API. As explained in the [OpenAI Platform documentation](https://platform.openai.com/docs/guides/reasoning?api-mode=responses#get-started-with-reasoning), this can be set to:
...
If the model name starts with `"o"` (as in `"o3"` or `"o4-mini"`) or `"adom"`, reasoning is enabled by default when using the Responses API. As explained in the [OpenAI Platform documentation](https://platform.openai.com/docs/guides/reasoning?api-mode=responses#reasoning-summaries), this can be set to:
...
When set, Adom includes a `text` object in the request payload with the configured verbosity, for example: `"text": { "verbosity": "low" }`.
...
In general, Adom knows the context window for the most common OpenAI models, but if you are using a new model with an old version of the Adom CLI, then you can use `model_context_window` to tell Adom what value to use to determine how much context is left during a conversation.

### model_max_output_tokens

This is analogous to `model_context_window`, but for the maximum number of output tokens for the model.

> See also [`adom exec`](./exec.md) to see how these model settings influence non-interactive runs.

## Execution environment

### approval_policy

Determines when the user should be prompted to approve whether Adom can execute a command:

```toml
# Adom has hardcoded logic that defines a set of "trusted" commands.
# Setting the approval_policy to `untrusted` means that Adom will prompt the
# user before running a command not in the "trusted" set.
#
# See https://github.com/openai/adom/issues/1260 for the plan to enable
...
```toml
# If the command fails when run in the sandbox, Adom asks for permission to
...
```toml
# User is never prompted: if the command fails, Adom will automatically try
...
Adom executes model-generated shell commands inside an OS-level sandbox.
...
A more relaxed policy is `workspace-write`. When specified, the current working directory for the Adom task will be writable (as well as `$TMPDIR` on macOS). Note that the CLI defaults to using the directory where it was spawned as `cwd`, though this can be overridden using `--cwd/-C`.

On macOS (and soon Linux), all writable roots (including `cwd`) that contain a `.git/` folder _as an immediate child_ will configure the `.git/` folder to be read-only while the rest of the Git repository will be writable. This means that commands like `git commit` will fail, by default (as it entails writing to `.git/`), and will require Adom to ask for permission.
...
# By default, the cwd for the Adom session will be writable as well as $TMPDIR
...
This is reasonable to use if Adom is running in an environment that provides its own sandboxing (such as a Docker container) such that further sandboxing is unnecessary.

Though using this option may also be necessary if you try to use Adom in environments where its native sandboxing mechanisms are unsupported, such as older Linux kernels or on Windows.

### tools.\*

Use the optional `[tools]` table to toggle built-in tools that the agent may call. `web_search` is now enabled by default, while `view_image` also starts enabled:
...
`web_search` is also recognized under the legacy name `web_search_request`. The `view_image` toggle is useful when you want to include screenshots or diagrams from your repo without pasting them manually. Adom still respects sandboxing: it can only attach files inside the workspace roots you allow.

### approval_presets

Adom provides three main Approval Presets:

- Read Only: Adom can read files and answer questions; edits, running commands, and network access require approval.
- Auto: Adom can read files, make edits, and run commands in the workspace without approval; asks for approval outside the workspace or for network access.
- Full Access: Full disk and network access without prompts; extremely risky.

You can further customize how Adom runs at the command line using the `--ask-for-approval` and `--sandbox` options.

> See also [Sandbox & approvals](./sandbox.md) for in-depth examples and platform-specific behaviour.

### shell_environment_policy

Adom spawns subprocesses (e.g. when executing a `local_shell` tool-call suggested by the assistant). By default it now passes **your full environment** to those subprocesses. You can tune this behavior via the **`shell_environment_policy`** block in `config.toml`:
...
| `ignore_default_excludes` | boolean              | `false` | When `false`, Adom removes any var whose **name** contains `KEY`, `SECRET`, or `TOKEN` (case-insensitive) before other rules run.              |
...
Currently, `ADOM_SANDBOX_NETWORK_DISABLED=1` is also added to the environment, assuming network is disabled. This is not configurable.

## MCP integration

### mcp_servers

You can configure Adom to use [MCP servers](https://modelcontextprotocol.io/about) to give Adom access to external applications, resources, or services.
...
# https://github.com/openai/adom/blob/main/adom-rs/rmcp-client/src/utils.rs#L82
env = { "API_KEY" = "value" }
# or
...
[Streamable HTTP servers](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports#streamable-http) enable Adom to talk to resources that are accessed via a http url (either on localhost or another domain).
...
After enabling it, run `adom mcp login <server-name>` when the server supports OAuth.
...
When both `enabled_tools` and `disabled_tools` are specified, Adom first restricts the server to the allow-list and then removes any tools that appear in the deny-list.
...
adom mcp add docs -- docs-server --port 4000
...
adom mcp get docs
adom mcp get docs --json

# Remove a server
adom mcp remove docs

# Log in to a streamable HTTP server that supports oauth
adom mcp login SERVER_NAME

# Log out from a streamable HTTP server that supports oauth
adom mcp logout SERVER_NAME
```

### Examples of useful MCPs

There is an ever growing list of useful MCP servers that can be helpful while you are working with Adom.
...
- [Sentry](https://docs.sentry.io/product/sentry-mcp/#adom) — access to your Sentry logs
...
Adom can emit [OpenTelemetry](https://opentelemetry.io/) **log events** that
...
Adom tags every exported event with `service.name = $ORIGINATOR` (the same
value sent in the `originator` header, `adom_cli_rs` by default), the CLI
version, and an `env` attribute so downstream collectors can distinguish
dev/staging/prod traffic. Only telemetry produced inside the `adom_otel`
...
With OTEL enabled Adom emits the following event types (in addition to the
metadata above):

- `adom.conversation_starts`
  - `provider_name`
...
If you build Adom from source the OTEL crate is still behind an `otel` feature
...
Specify a program that will be executed to get notified about events generated by Adom. Note that the program will receive the notification argument as a string of JSON, e.g.:
...
`"thread-id"` contains a string that identifies the Adom session that produced the notification; you can use it to correlate multiple turns that belong to the same task.
    ...
    match notification_type := notification.get("type"):
        case "agent-turn-complete":
            ...
            if assistant_message:
                title = f"Adom: {assistant_message}"
            else:
                title = "Adom: Turn Complete!"
...
To have Adom use this script for notifications, you would configure it via `notify` in `~/.adom/config.toml` using the appropriate path to `notify.py` on your computer:

```toml
notify = ["python3", "/Users/mbolin/.adom/notify.py"]
```

> [!NOTE]
> Use `notify` for automation and integrations: Adom invokes your external program with a single JSON argument for each event, independent of the TUI. If you only want lightweight desktop notifications while using the TUI, prefer `tui.notifications`, which uses terminal escape codes and requires no external program. You can enable both; `tui.notifications` covers in‑TUI alerts (e.g., approval prompts), while `notify` is best for system‑level hooks or custom notifiers. Currently, `notify` emits only `agent-turn-complete`, whereas `tui.notifications` supports `agent-turn-complete` and `approval-requested` with optional filtering.

### hide_agent_reasoning

Adom intermittently emits "reasoning" events that show the model's internal "thinking" before it produces a final answer. Some users may find these events distracting, especially in CI logs or minimal terminal output.
...
4. the default value that comes with Adom CLI (i.e., Adom CLI defaults to `gpt-5-adom`)

### history

By default, Adom CLI records messages sent to the model in `$ADOM_HOME/history.jsonl`. Note that on UNIX, the file permissions are set to `o600`, so it should only be readable and writable by the owner.
...
Currently, `"vscode"` is the default, though Adom does not verify VS Code is installed. As such, `file_opener` may default to `"none"` or something else in the future.
...
> See also [AGENTS.md discovery](./agents_md.md) for how Adom locates these files during a session.
...
> [!NOTE]
> Adom emits desktop notifications using terminal escape codes. Not all terminals support these (notably, macOS Terminal.app and VS Code's terminal do not support custom notifications. iTerm2, Ghostty and WezTerm do support these notifications).
...
To force users on a given machine to use a specific login method or workspace, use a combination of [managed configurations](https://developers.openai.com/adom/security#managed-configuration) as well as either or both of the following fields:
...
# flow and the id will be validated during the oauth callback as well as every time Adom starts.
forced_chatgpt_workspace_id = "00000000-0000-0000-0000-000000000000"
```

If the active credentials don't match the config, the user will be logged out and Adom will exit.
...
- `file` (default) – Store credentials in `auth.json` under `$ADOM_HOME`.
...
- `auto` – Save credentials to the operating system keyring when available; otherwise, fall back to `auth.json` under `$ADOM_HOME`.

## Config reference

| Key                                              | Type / Values                                                     | Notes                                                                                                                      |
| ------------------------------------------------ | ----------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `model`                                          | string                                                            | Model to use (e.g., `gpt-5-adom`).                                                                                        |
...
| `mcp_servers.<id>.enabled`                       | boolean                                                           | When false, Adom skips starting the server (default: true).                                                               |
...
| `tools.view_image`                               | boolean                                                           | Enable or disable the `view_image` tool so Adom can attach local image files from the workspace (default: true).          |
| `forced_login_method`                            | `chatgpt` \| `api`                                                | Only allow Adom to be used with ChatGPT or API keys.                                                                      |
| `forced_chatgpt_workspace_id`                    | string (uuid)                                                     | Only allow Adom to be used with the specified ChatGPT workspace.                                                          |
