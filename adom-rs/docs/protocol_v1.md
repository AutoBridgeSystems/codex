...
These are entities exit on the adom backend. The intent of this section is to establish vocabulary and construct a shared mental model for the `Adom` core system.

0. `Model`
   - In our case, this is the Responses REST API
1. `Adom`
   - The core engine of adom
...
2. `Session`
   - The `Adom`'s current configuration and state
   - `Adom` starts with no `Session`, and it is initialized by `Op::ConfigureSession`, which should be the first message sent by the UI.
...
3. `Task`
   - A `Task` is `Adom` executing work in response to user input.
...
4. `Turn`
   - One cycle of iteration in a `Task`, consists of:
     ...
     - `Adom` then executes command(s), applies patch(es), and outputs message(s) returned by the `Model`
...
The term "UI" is used to refer to the application driving `Adom`. This may be the CLI / TUI chat-like interface that users operate, or it may be a GUI interface like a VSCode extension. The UI is external to `Adom`, as `Adom` is intended to be operated by arbitrary UI implementations.
...
Since only 1 `Task` can be run at a time, for parallel tasks it is recommended that a single `Adom` be run for each thread of work.
...
- `Submission`
  - These are messages sent on the `SQ` (UI -> `Adom`)
...
- `Event`
  - These are messages sent on the `EQ` (`Adom` -> UI)
...
sequenceDiagram
    ...
    user->>adom: Op::ConfigureSession
    adom-->>session: create session
    adom->>user: Event::SessionConfigured
