...
//!
//! If the keyring is not available or fails, we fall back to ADOM_HOME/.credentials.json which is consistent with other coding CLI agents.
...
use crate::find_adom_home::find_adom_home;

const KEYRING_SERVICE: &str = "Adom MCP Credentials";
...
/// Determine where Adom should store and read MCP credentials.
...
mod tests {
    ...
    impl TempAdomHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create ADOM_HOME temp dir");
            unsafe {
                std::env::set_var("ADOM_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempAdomHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("ADOM_HOME");
            }
        }
    }
