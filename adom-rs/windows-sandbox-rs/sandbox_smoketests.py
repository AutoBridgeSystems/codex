...
# Run a suite of smoke tests against the Windows sandbox via the Adom CLI
...
def _resolve_adom_cmd() -> List[str]:
    """Resolve the Adom CLI to invoke `adom sandbox windows`.

    Prefer `adom` on PATH; if not found, try common local build locations.
    Returns the argv prefix to run Adom.
    """
    # 1) Prefer PATH
    try:
        cp = subprocess.run(["where", "adom"], stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, text=True)
        if cp.returncode == 0:
    ...
    candidates = [
        ws_root / "target" / "release" / "adom.exe",
        ws_root / "target" / "debug" / "adom.exe",
    ]
    if cargo_target:
        candidates.extend([
            Path(cargo_target) / "release" / "adom.exe",
            Path(cargo_target) / "debug" / "adom.exe",
        ])
    for p in candidates:
        if p.exists():
            return [str(p)]
    ...
    raise FileNotFoundError(
        "Adom CLI not found. Build it first, e.g.\n"
        "  cargo build -p adom-cli --release\n"
        "or for debug:\n"
        "  cargo build -p adom-cli\n"
    )

ADOM_CMD = _resolve_adom_cmd()
TIMEOUT_SEC = 20
...
def run_sbx(policy: str, cmd_argv: List[str], cwd: Path, env_extra: Optional[dict] = None) -> Tuple[int, str, str]:
    ...
    argv = [*ADOM_CMD, "sandbox", "windows", *policy_flags, "--", *cmd_argv]
    print(cmd_argv)
