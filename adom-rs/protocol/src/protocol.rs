//! Defines the protocol for a Adom session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Adom:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::adom_conversation::AdomConversation`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// Request Adom to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request a code review from the agent.
    Review { review_request: ReviewRequest },

    /// Request to shut down adom instance.
    Shutdown,
...
/// running the command proposed by Adom.
#[derive(
    Debug,
    Clone,
    Copy,
