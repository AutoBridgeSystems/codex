...
async fn truncate_function_error_trims_respond_to_model() -> Result<()> {
    ...
    let mut builder = test_adom().with_config(|config| {
        // Use the test model that wires function tools like grep_files
        config.model = "test-gpt-5-adom".to_string();
        config.model_family =
            find_family_for_model("test-gpt-5-adom").expect("model family for test model");
    });
...
async fn tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    let mut builder = test_adom().with_config(|config| {
        config.model = "gpt-5-adom".to_string();
        config.model_family =
            find_family_for_model("gpt-5-adom").expect("gpt-5-adom is a model family");
    });
...
async fn mcp_tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    let mut builder = test_adom().with_config(move |config| {
        config.features.enable(Feature::RmcpClient);
        config.mcp_servers.insert(
            server_name.to_string(),
            adom_core::config::types::McpServerConfig {
                transport: adom_core::config::types::McpServerTransportConfig::Stdio {
