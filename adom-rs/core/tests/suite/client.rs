...
use adom_core::ContentItem;
use adom_core::ConversationManager;
use adom_core::LocalShellAction;
use adom_core::LocalShellExecAction;
use adom_core::LocalShellStatus;
use adom_core::ModelClient;
use adom_core::ModelProviderInfo;
use adom_core::NewConversation;
use adom_core::Prompt;
use adom_core::ResponseEvent;
use adom_core::ResponseItem;
use adom_core::WireApi;
use adom_core::auth::AuthCredentialsStoreMode;
use adom_core::built_in_model_providers;
use adom_core::error::AdomErr;
use adom_core::model_family::find_family_for_model;
use adom_core::protocol::EventMsg;
use adom_core::protocol::Op;
use adom_core::protocol::SessionSource;
use adom_otel::otel_event_manager::OtelEventManager;
use adom_protocol::ConversationId;
use adom_protocol::models::ReasoningItemContent;
use adom_protocol::models::ReasoningItemReasoningSummary;
use adom_protocol::models::WebSearchAction;
use adom_protocol::user_input::UserInput;
use core_test_support::load_default_config_for_test;
use core_test_support::load_sse_fixture_with_id;
use core_test_support::responses;
use core_test_support::skip_if_no_network;
use core_test_support::test_adom::TestAdom;
use core_test_support::test_adom::test_adom;
...
/// Writes an `auth.json` into the provided `adom_home` with the specified parameters.
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = adom_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![adom_protocol::models::ContentItem::InputText {
    ...
    let prior_system = adom_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![adom_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = adom_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Adom to resume from our file
    ...
    let auth_manager =
        adom_core::AuthManager::from_auth_for_testing(AdomAuth::from_api_key("Test API Key"));
    let NewConversation {
        conversation: adom,
        session_configured,
        ..
    ...
    adom
        ...
        .await
        .unwrap();
    wait_for_event(&adom, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    assert_eq!(request_originator.to_str().unwrap(), "adom_cli_rs");
    assert_eq!(
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    let model_provider = ModelProviderInfo {
        base_url: Some(format!("{}/api/adom", server.uri())),
        ..built_in_model_providers()["openai"].clone()
    };
    ...
    let mut config = load_default_config_for_test(&adom_home);
    config.model_provider = model_provider;
    let conversation_manager = ConversationManager::with_auth(create_dummy_adom_auth());
    let NewConversation {
        conversation: adom,
        conversation_id,
        session_configured: _,
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager =
        match AdomAuth::from_auth_storage(adom_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => adom_core::AuthManager::from_auth_for_testing(auth),
            Ok(None) => panic!("No AdomAuth found in adom_home"),
            Err(e) => panic!("Failed to load AdomAuth: {e}"),
        };
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let client = ModelClient::new(
        ...
        summary,
        conversation_id,
        adom_protocol::protocol::SessionSource::Exec,
    );
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-adom-primary-used-percent", "12.5")
        .insert_header("x-adom-secondary-used-percent", "40.0")
        .insert_header("x-adom-primary-window-minutes", "10")
        .insert_header("x-adom-secondary-window-minutes", "60")
        .insert_header("x-adom-primary-reset-at", "1704069000")
        .insert_header("x-adom-secondary-reset-at", "1704074400")
        .set_body_raw(sse_body, "text/event-stream");
    ...
    let conversation_manager = ConversationManager::with_auth(AdomAuth::from_api_key("test"));
    let adom = conversation_manager
        .new_conversation(config)
        .await
    ...
    let first_token_event =
        wait_for_event(&adom, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5-adom in tests → 95% usable context window
    ...
    wait_for_event(&adom, |msg| matches!(msg, EventMsg::TaskComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-adom-primary-used-percent", "100.0")
        .insert_header("x-adom-secondary-used-percent", "87.5")
        .insert_header("x-adom-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-adom-primary-window-minutes", "15")
        .insert_header("x-adom-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_adom();
    let adom_fixture = builder.build(&server).await?;
    let adom = adom_fixture.adom.clone();
    ...
    let token_event = wait_for_event(&adom, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&adom, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&adom, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&adom, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = AdomErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_adom_auth() -> AdomAuth {
    AdomAuth::create_dummy_chatgpt_auth_for_testing()
}
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Adom sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
    ...
    // Init session with isolated adom home.
    let adom_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&adom_home);
    config.model_provider = model_provider;
