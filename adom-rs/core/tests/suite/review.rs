...
use adom_core::protocol::RolloutItem;
use adom_core::protocol::RolloutLine;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Adom sandbox network restrictions.
    skip_if_no_network!();
    ...
    let adom = new_conversation_for_server(&server, &adom_home, |_| {}).await;
    ...
    let _entered = wait_for_event(&adom, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&adom, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&adom, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = adom.rollout_path();
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&adom, |event| match event {
        ...
        EventMsg::ItemCompleted(ev) => match &ev.item {
            adom_protocol::items::TurnItem::AgentMessage(_) => {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let adom = new_conversation_for_server(&server, &adom_home, |cfg| {
    ...
    // Wait for completion
    let _entered = wait_for_event(&adom, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&adom, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = adom_home.path().join("resume.jsonl");
    {
        ...
        let user = adom_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = adom_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let adom =
        resume_conversation_for_server(&server, &adom_home, session_file.clone(), |_| {}).await;
