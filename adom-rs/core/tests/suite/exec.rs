...
use adom_core::exec::ExecToolCallOutput;
use adom_core::exec::SandboxType;
use adom_core::exec::process_exec_tool_call;
use adom_core::protocol::SandboxPolicy;
use adom_core::spawn::ADOM_SANDBOX_ENV_VAR;
use tempfile::TempDir;

use adom_core::error::Result;

use adom_core::get_platform_sandbox;

fn skip_test() -> bool {
    if std::env::var(ADOM_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{ADOM_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }
