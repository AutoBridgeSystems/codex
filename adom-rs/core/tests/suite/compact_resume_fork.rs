...
//! request payload that Adom would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(ADOM_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn start_test_conversation(
    server: &MockServer,
) -> (TempDir, Config, ConversationManager, Arc<AdomConversation>) {
    ...
    let manager = ConversationManager::with_auth(AdomAuth::from_api_key("dummy"));
...
async fn user_turn(conversation: &Arc<AdomConversation>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<AdomConversation>) {
    conversation
        .submit(Op::Compact)
        .await
...
async fn fetch_conversation_path(conversation: &Arc<AdomConversation>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager =
        adom_core::AuthManager::from_auth_for_testing(AdomAuth::from_api_key("dummy"));
    let NewConversation { conversation, .. } = manager
