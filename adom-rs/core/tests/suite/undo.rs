...
use core_test_support::test_adom::TestAdomHarness;
use core_test_support::wait_for_event_match;
use pretty_assertions::assert_eq;

#[allow(clippy::expect_used)]
async fn undo_harness() -> Result<TestAdomHarness> {
    TestAdomHarness::with_config(|config: &mut Config| {
...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.name", "Adom Tests"])?;
    git(path, &["config", "user.email", "adom-tests@example.com"])?;

    // Create README.txt
    let readme_path = path.join("README.txt");
    fs::write(&readme_path, "Test repository initialized by Adom.\n")?;
...
async fn invoke_undo(adom: &Arc<AdomConversation>) -> Result<UndoCompletedEvent> {
    adom.submit(Op::Undo).await?;
    let event = wait_for_event_match(adom, |msg| match msg {
...
async fn expect_successful_undo(adom: &Arc<AdomConversation>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(adom).await?;
    assert!(
        event.success,
...
async fn expect_failed_undo(adom: &Arc<AdomConversation>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(adom).await?;
    assert!(
        !event.success,
...
async fn undo_removes_new_file_created_during_turn() -> Result<()> {
    ...
    let adom = Arc::clone(&harness.test().adom);
    let completed = expect_successful_undo(&adom).await?;
...
async fn undo_sequential_turns_consumes_snapshots() -> Result<()> {
    ...
    let adom = Arc::clone(&harness.test().adom);
    expect_successful_undo(&adom).await?;
    assert_eq!(fs::read_to_string(&story)?, "turn two\n");
