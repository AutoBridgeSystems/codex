...
fn write_minimal_rollout_with_id(adom_home: &Path, id: Uuid) -> PathBuf {
    let sessions = adom_home.join("sessions/2024/01/01");
...
#[tokio::test]
async fn find_handles_gitignore_covering_adom_home_directory() {
    let repo = TempDir::new().unwrap();
    let adom_home = repo.path().join(".adom");
    std::fs::create_dir_all(&adom_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".adom/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&adom_home, id);

    let found = find_conversation_path_by_id_str(&adom_home, &id.to_string())
        .await
        .unwrap();
