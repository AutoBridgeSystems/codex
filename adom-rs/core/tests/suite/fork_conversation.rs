...
async fn fork_conversation_twice_drops_to_first_message() {
    ...
    // Configure Adom to use the mock server.
    ...
    for text in ["first", "second", "third"] {
        adom
            ...
            .await
            .unwrap();
        let _ = wait_for_event(&adom, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;
    }

    // Request history from the base conversation to obtain rollout path.
    let base_path = adom.rollout_path();
    ...
    let fork1_path = adom_fork1.rollout_path();
    ...
    let fork2_path = adom_fork2.rollout_path();
