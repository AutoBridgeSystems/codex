...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    let adom = test_adom().build(&server).await.unwrap().adom;
    ...
    wait_for_event(&adom, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    adom.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&adom, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
}
...
async fn interrupt_tool_records_history_entries() {
    ...
    let fixture = test_adom().build(&server).await.unwrap();
    let adom = Arc::clone(&fixture.adom);
