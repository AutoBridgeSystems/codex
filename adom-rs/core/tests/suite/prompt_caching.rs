...
use adom_core::shell::Shell;
use adom_core::shell::default_user_shell;
...
async fn adom_mini_latest_tools() {
    skip_if_no_network!();
    ...
    config.model = "adom-mini-latest".to_string();
    config.model_family = find_family_for_model("adom-mini-latest").unwrap();
