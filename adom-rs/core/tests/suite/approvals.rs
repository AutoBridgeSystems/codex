...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestAdom) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestAdom, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.adom, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestAdom) {
    let event = wait_for_event(&test.adom, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TaskComplete(_)
...
async fn wait_for_completion(test: &TestAdom) {
    wait_for_event(&test.adom, |event| {
...
fn scenarios() -> Vec<ScenarioSpec> {
    ...
    vec![
        ...
        ScenarioSpec {
            ...
            with_escalated_permissions: false,
            requires_apply_patch_tool: true,
            model_override: Some("gpt-5-adom"),
            outcome: Outcome::Auto,
            expectation: Expectation::PatchApplied {
                target: TargetPath::Workspace("apply_patch_function.txt"),
                content: "function-apply-patch",
