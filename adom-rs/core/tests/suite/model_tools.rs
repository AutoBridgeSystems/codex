...
async fn model_selects_expected_tools() {
    ...
    let adom_tools = collect_tool_identifiers_for_model("adom-mini-latest").await;
    assert_eq!(
        adom_tools,
        vec![
        ...
        "adom-mini-latest should expose the local shell tool",
    );
    ...
    let gpt5_adom_tools = collect_tool_identifiers_for_model("gpt-5-adom").await;
    assert_eq!(
        gpt5_adom_tools,
        vec![
            "shell".to_string(),
        ...
        "gpt-5-adom should expose the apply_patch tool",
    );
}
