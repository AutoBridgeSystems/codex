...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Adom.
    ...
    adom.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&adom, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Adom to flush rollout entries before inspecting the file.
    adom.submit(Op::Shutdown).await.unwrap();
...
async fn manual_compact_uses_custom_prompt() {
    ...
    adom.submit(Op::Compact).await.expect("trigger compact");
    let warning_event = wait_for_event(&adom, |ev| matches!(ev, EventMsg::Warning(_))).await;
    let EventMsg::Warning(WarningEvent { message }) = warning_event else {
        panic!("expected warning event after compact");
    };
...
async fn manual_compact_retries_after_context_window_error() {
    ...
    set_test_compact_prompt(&mut config);
    config.model_auto_compact_token_limit = Some(200_000);
    let adom = ConversationManager::with_auth(AdomAuth::from_api_key("dummy"))
        .new_conversation(config)
        .await
        .unwrap()
        .conversation;
    ...
    let EventMsg::BackgroundEvent(event) =
        wait_for_event(&adom, |ev| matches!(ev, EventMsg::BackgroundEvent(_))).await
    else {
...
async fn auto_compact_allows_multiple_attempts_when_interleaved_with_other_turn_events() {
    ...
    loop {
        let event = adom.next_event().await.unwrap();
