...
use adom_core::protocol::GitInfo;
...
/// 2. Configures adom to use this mock server via a custom provider
...
async fn exec_cli_applies_experimental_instructions_file() {
    ...
    // Adom to use the Responses API with the dummy env var.
    let provider_override = format!(
...
/// 2. Configures adom to read from this fixture via ADOM_RS_SSE_FIXTURE env var
...
async fn responses_api_stream_cli() {
    ...
    cmd.env("ADOM_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("ADOM_RS_SSE_FIXTURE", fixture)
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the adom CLI and invoke `exec`, which is what records a session.
    let bin = cargo_bin("adom");
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("ADOM_HOME", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("ADOM_RS_SSE_FIXTURE", &fixture)
        .env("OPENAI_BASE_URL", "http://unused.local");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume adom-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = adom_core::git_info::collect_git_info(&git_repo).await;
