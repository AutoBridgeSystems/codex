...
async fn run_turn(test: &TestAdom, prompt: &str) -> anyhow::Result<()> {
    ...
    wait_for_event(&test.adom, |ev| matches!(ev, EventMsg::TaskComplete(_))).await;

    Ok(())
}

async fn run_turn_and_measure(test: &TestAdom, prompt: &str) -> anyhow::Result<Duration> {
...
async fn build_adom_with_test_tool(server: &wiremock::MockServer) -> anyhow::Result<TestAdom> {
    let mut builder = test_adom().with_config(|config| {
        config.model = "test-gpt-5-adom".to_string();
        config.model_family =
            find_family_for_model("test-gpt-5-adom").expect("test-gpt-5-adom model family");
    });
...
async fn read_file_tools_run_in_parallel() -> anyhow::Result<()> {
    ...
    let test = build_adom_with_test_tool(&server).await?;
...
async fn non_parallel_tools_run_serially() -> anyhow::Result<()> {
    ...
    let server = start_mock_server().await;
    let test = test_adom().build(&server).await?;
