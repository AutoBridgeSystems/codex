...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    let conversation_manager =
        ConversationManager::with_auth(adom_core::AdomAuth::from_api_key("dummy"));
    let NewConversation {
        conversation: adom,
        ..
    ...
    adom
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&adom, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _ = wait_for_event(&adom, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;
    adom.submit(Op::Interrupt).await.unwrap();

    // Expect a TurnAborted(Interrupted) notification.
    let msg = wait_for_event(&adom, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
