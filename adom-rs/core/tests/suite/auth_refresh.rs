...
use adom_core::auth::AuthDotJson;
use adom_core::auth::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
use adom_core::auth::RefreshTokenError;
use adom_core::auth::load_auth_dot_json;
use adom_core::auth::save_auth;
use adom_core::error::RefreshTokenFailedReason;
use adom_core::token_data::IdTokenInfo;
use adom_core::token_data::TokenData;
...
impl RefreshTokenTestContext {
    fn new(server: &MockServer) -> Result<Self> {
        ...
        let auth = AdomAuth::from_auth_storage(adom_home.path(), AuthCredentialsStoreMode::File)?
    ...
    fn load_auth(&self) -> Result<AuthDotJson> {
        load_auth_dot_json(self.adom_home.path(), AuthCredentialsStoreMode::File)
