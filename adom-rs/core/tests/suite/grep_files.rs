...
const MODEL_WITH_TOOL: &str = "test-gpt-5-adom";
...
async fn grep_files_tool_collects_matches() -> Result<()> {
    ...
    let test = build_test_adom(&server).await?;
...
async fn build_test_adom(server: &wiremock::MockServer) -> Result<TestAdom> {
...
async fn submit_turn(test: &TestAdom, prompt: &str) -> Result<()> {
