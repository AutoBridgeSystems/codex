...
async fn adom_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    adom_returns_json_result("gpt-5".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn adom_returns_json_result_for_gpt5_adom() -> anyhow::Result<()> {
    adom_returns_json_result("gpt-5-adom".to_string()).await
}

async fn adom_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("adom_output_schema".into()))
    ...
    let TestAdom { adom, cwd, .. } = test_adom().build(&server).await?;
    ...
    let message = wait_for_event(&adom, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
