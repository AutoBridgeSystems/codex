...
/// avoids clobbering a developer’s real `~/.adom`.
pub fn load_default_config_for_test(adom_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        adom_linux_sandbox_exe: Some(cargo_bin("adom-linux-sandbox")),
...
pub async fn wait_for_event<F>(
    adom: &AdomConversation,
    predicate: F,
) -> adom_core::protocol::EventMsg
where
    F: FnMut(&adom_core::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(adom, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(adom: &AdomConversation, matcher: F) -> T
where
    F: Fn(&adom_core::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(adom, |ev| matcher(ev).is_some()).await;
    matcher(&ev).unwrap()
}
    ...
    F: FnMut(&adom_core::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(5)), adom.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    adom_core::spawn::ADOM_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    adom_core::spawn::ADOM_SANDBOX_NETWORK_DISABLED_ENV_VAR
}
