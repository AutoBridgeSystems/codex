...
use adom_protocol::protocol::ApplyPatchApprovalRequestEvent;
use adom_protocol::protocol::Event;
use adom_protocol::protocol::EventMsg;
use adom_protocol::protocol::ExecApprovalRequestEvent;
use adom_protocol::protocol::Op;
use adom_protocol::protocol::SessionSource;
use adom_protocol::protocol::SubAgentSource;
use adom_protocol::protocol::Submission;
use adom_protocol::user_input::UserInput;
use tokio_util::sync::CancellationToken;

use crate::AuthManager;
use crate::adom::Adom;
use crate::adom::AdomSpawnOk;
use crate::adom::SUBMISSION_CHANNEL_CAPACITY;
...
/// Start an interactive sub-Adom conversation and return IO channels.
///
/// The returned `events_rx` yields non-approval events emitted by the sub-agent.
/// Approval requests are handled via `parent_session` and are not surfaced.
/// The returned `ops_tx` allows the caller to submit additional `Op`s to the sub-agent.
pub(crate) async fn run_adom_conversation_interactive(
    config: Config,
    ...
    let AdomSpawnOk { adom, .. } = Adom::spawn(
        config,
        auth_manager,
    ...
    let adom_for_events = Arc::clone(&adom);
    ...
    let adom_for_ops = Arc::clone(&adom);
    tokio::spawn(async move {
        forward_ops(adom_for_ops, rx_ops, cancel_token_ops).await;
    });
...
pub(crate) async fn run_adom_conversation_one_shot(
    config: Config,
    auth_manager: Arc<AuthManager>,
    input: Vec<UserInput>,
    parent_session: Arc<Session>,
    ...
    let io = run_adom_conversation_interactive(
        config,
        auth_manager,
        parent_session,
        parent_ctx,
...
async fn forward_events(
    ...
    while let Ok(event) = adom.next_event().await {
        match event {
...
async fn handle_exec_approval(
    ...
    let _ = adom.submit(Op::ExecApproval { id, decision }).await;
}
...
async fn handle_patch_approval(
    ...
    .await;
    let _ = adom.submit(Op::PatchApproval { id, decision }).await;
}
...
async fn await_approval_with_cancel<F>(
    ...
    sub_id: &str,
    cancel_token: &CancellationToken,
) -> adom_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = adom_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            parent_session
                .notify_approval(sub_id, adom_protocol::protocol::ReviewDecision::Abort)
                .await;
            adom_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
