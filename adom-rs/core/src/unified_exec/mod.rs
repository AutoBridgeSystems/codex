...
mod tests {
    ...
    async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
        ...
        write_stdin(
            &session,
            session_id,
            "export ADOM_INTERACTIVE_SHELL_VAR=adom\n",
            Some(2_500),
        )
        .await?;
        ...
        let out_2 = write_stdin(
            &session,
            session_id,
            "echo $ADOM_INTERACTIVE_SHELL_VAR\n",
            Some(2_500),
        )
        .await?;
        assert!(
            out_2.output.contains("adom"),
    ...
    async fn unified_exec_timeouts() -> anyhow::Result<()> {
        ...
        let out_2 = write_stdin(
            &session,
            session_id,
            "sleep 5 && echo $ADOM_INTERACTIVE_SHELL_VAR\n",
            Some(10),
        )
        .await?;
        assert!(
    ...
    async fn requests_with_large_timeout_are_capped() -> anyhow::Result<()> {
        ...
        let result = exec_command(&session, &turn, "echo adom", Some(120_000)).await?;

        assert!(result.session_id.is_none());
        assert!(result.output.contains("adom"));
    ...
    async fn completed_commands_do_not_persist_sessions() -> anyhow::Result<()> {
        let (session, turn) = test_session_and_turn();
        let result = exec_command(&session, &turn, "echo adom", Some(2_500)).await?;
