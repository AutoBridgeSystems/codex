...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, AdomErr>> {
        ...
        async move {
            match handle.await {
                ...
                Ok(Err(FunctionCallError::Fatal(message))) => Err(AdomErr::Fatal(message)),
                Ok(Err(other)) => Err(AdomErr::Fatal(other.to_string())),
                Err(err) => Err(AdomErr::Fatal(format!(
