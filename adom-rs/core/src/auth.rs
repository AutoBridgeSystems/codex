...
use crate::default_client::AdomHttpClient;
...
use adom_protocol::account::PlanType as AccountPlanType;
...
pub struct AdomAuth {
    ...
    pub(crate) client: AdomHttpClient,
}

impl PartialEq for AdomAuth {
...
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "ADOM_REFRESH_TOKEN_URL_OVERRIDE";
...
impl AdomAuth {
    ...
    ) -> std::io::Result<Option<AdomAuth>> {
        load_auth(adom_home, false, auth_credentials_store_mode)
    }
    ...
    fn from_api_key_with_client(api_key: &str, client: AdomHttpClient) -> Self {
        Self {
...
pub const ADOM_API_KEY_ENV_VAR: &str = "ADOM_API_KEY";
...
pub fn read_adom_api_key_from_env() -> Option<String> {
    env::var(ADOM_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

/// Delete the auth.json file inside `adom_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    adom_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(adom_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(adom_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_with_message(
    ...
    match logout(adom_home, auth_credentials_store_mode) {
...
fn load_auth(
    adom_home: &Path,
    enable_adom_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<AdomAuth>> {
    if enable_adom_api_key_env && let Some(api_key) = read_adom_api_key_from_env() {
        let client = crate::default_client::create_client();
        return Ok(Some(AdomAuth::from_api_key_with_client(
            api_key.as_str(),
            client,
        )));
    }
    ...
    if let Some(api_key) = &auth_json_api_key {
        return Ok(Some(AdomAuth::from_api_key_with_client(api_key, client)));
    }
...
mod tests {
    ...
    fn missing_auth_json_returns_none() {
        let dir = tempdir().unwrap();
        let auth = AdomAuth::from_auth_storage(dir.path(), AuthCredentialsStoreMode::File)
    ...
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let AdomAuth {
            ...
            storage: _,
            ..
        } = super::load_auth(adom_home.path(), false, AuthCredentialsStoreMode::File)
            .unwrap()
            .unwrap();
    ...
    fn write_auth_file(params: AuthFileParams, adom_home: &Path) -> std::io::Result<String> {
        let auth_file = get_auth_file(adom_home);
    ...
    async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
        let adom_home = tempdir().unwrap();
        login_with_api_key(adom_home.path(), "sk-test", AuthCredentialsStoreMode::File)
            .expect("seed api key");

        let config = build_config(adom_home.path(), Some(ForcedLoginMethod::Chatgpt), None);
        ...
        assert!(
            !adom_home.path().join("auth.json").exists(),
    ...
    async fn enforce_login_restrictions_logs_out_for_workspace_mismatch() {
        ...
        let config = build_config(adom_home.path(), None, Some("org_mine".to_string()));
    ...
    async fn enforce_login_restrictions_blocks_env_api_key_when_chatgpt_required() {
        let _guard = EnvVarGuard::set(ADOM_API_KEY_ENV_VAR, "sk-env");
        let adom_home = tempdir().unwrap();
    ...
    fn plan_type_maps_known_plan() {
        ...
        let auth = super::load_auth(adom_home.path(), false, AuthCredentialsStoreMode::File)
            .expect("load auth")
...
/// hands out cloned `AdomAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific AdomAuth, for testing only.
    pub fn from_auth_for_testing(auth: AdomAuth) -> Arc<Self> {
        let cached = CachedAuth { auth: Some(auth) };
        Arc::new(Self {
            adom_home: PathBuf::new(),
            inner: RwLock::new(cached),
            enable_adom_api_key_env: false,
            auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        })
    }

    /// Current cached auth (clone). May be `None` if not logged in or load failed.
    pub fn auth(&self) -> Option<AdomAuth> {
    ...
    fn auths_equal(a: &Option<AdomAuth>, b: &Option<AdomAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = super::auth::logout(&self.adom_home, self.auth_credentials_store_mode)?;
