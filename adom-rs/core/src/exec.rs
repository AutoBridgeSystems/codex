...
pub async fn process_exec_tool_call(
    ...
    let exec_env = manager
        .transform(
            ...
            sandbox_type,
            sandbox_cwd,
            adom_linux_sandbox_exe.as_ref(),
        )
        .map_err(AdomErr::from)?;
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_adom_home;
    use adom_windows_sandbox::run_windows_sandbox_capture;
    ...
    let logs_base_dir = find_adom_home().ok();
    ...
    let capture = match spawn_res {
        Ok(Ok(v)) => v,
        Ok(Err(err)) => {
            return Err(AdomErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, AdomErr>,
    sandbox_type: SandboxType,
    duration: Duration,
) -> Result<ExecToolCallOutput> {
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(AdomErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(AdomErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(AdomErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
                }));
            }
...
pub(crate) mod errors {
    ...
    impl From<SandboxTransformError> for AdomErr {
        fn from(err: SandboxTransformError) -> Self {
            match err {
                SandboxTransformError::MissingLinuxSandboxExecutable => {
                    AdomErr::LandlockSandboxExecutableNotProvided
                }
                #[cfg(not(target_os = "macos"))]
                SandboxTransformError::SeatbeltUnavailable => AdomErr::UnsupportedOperation(
