...
use adom_app_server_protocol::Tools;
use adom_app_server_protocol::UserSavedConfig;
use adom_protocol::config_types::ForcedLoginMethod;
use adom_protocol::config_types::ReasoningEffort;
use adom_protocol::config_types::ReasoningSummary;
use adom_protocol::config_types::SandboxMode;
use adom_protocol::config_types::Verbosity;
use adom_rmcp_client::OAuthCredentialsStoreMode;
use dirs::home_dir;
...
pub const OPENAI_DEFAULT_MODEL: &str = "gpt-5-adom";
const OPENAI_DEFAULT_REVIEW_MODEL: &str = "gpt-5-adom";
pub const GPT_5_ADOM_MEDIUM_MODEL: &str = "gpt-5-adom";
...
pub struct Config {
    ...
    /// Model used specifically for review sessions. Defaults to "gpt-5-adom".
    ...
    /// Optional external notifier command. When set, Adom will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Adom
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.adom/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Adom"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Adom '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Adom home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Adom can reach out to for tool calls.
    pub mcp_servers: HashMap<String, McpServerConfig>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Adom unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/adom/blob/main/adom-rs/rmcp-client/src/oauth.rs#L2
    /// file: ADOM_HOME/.credentials.json
    ///       This file will be readable to Adom and other applications running as the same user.
    ...
    /// Directory containing all Adom state (defaults to `~/.adom` but can be
    /// overridden by the `ADOM_HOME` environment variable).
    pub adom_home: PathBuf,

    /// Settings that govern if and what will be written to `~/.adom/history.jsonl`.
    pub history: History,
    ...
    /// Path to the `adom-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `adom-linux-sandbox`.
    pub adom_linux_sandbox_exe: Option<PathBuf>,
...
impl Config {
    pub async fn load_with_cli_overrides(
        ...
        Self::load_from_base_config_with_overrides(cfg, overrides, adom_home)
    }
}
...
async fn load_resolved_config(
    ...
    let layers = load_config_layers_with_overrides(adom_home, overrides).await?;
...
pub async fn load_global_mcp_servers(
    ...
    let root_value = load_config_as_toml(adom_home).await?;
...
/// Patch `ADOM_HOME/config.toml` project state.
/// Use with caution.
pub fn set_project_trusted(adom_home: &Path, project_path: &Path) -> anyhow::Result<()> {
    use crate::config::edit::ConfigEditsBuilder;

    ConfigEditsBuilder::new(adom_home)
...
/// Base config deserialized from ~/.adom/config.toml.
...
pub struct ConfigToml {
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          https://github.com/openai/adom/blob/main/adom-rs/rmcp-client/src/oauth.rs#L2
    /// file: Use a file in the Adom home directory.
...
impl Config {
    ...
    pub fn load_from_base_config_with_overrides(
        ...
        let user_instructions = Self::load_instructions(Some(&adom_home));
    ...
    fn load_instructions(adom_dir: Option<&Path>) -> Option<String> {
        let base = adom_dir?;
...
/// Returns the path to the Adom configuration directory, which can be
/// specified by the `ADOM_HOME` environment variable. If not set, defaults to
/// `~/.adom`.
///
/// - If `ADOM_HOME` is set, the value will be canonicalized and this
///   function will Err if the path does not exist.
/// - If `ADOM_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_adom_home() -> std::io::Result<PathBuf> {
    // Honor the `ADOM_HOME` environment variable when it is set to allow users
    // (and tests) to override the default location.
    if let Ok(val) = std::env::var("ADOM_HOME")
        && !val.is_empty()
    {
...
/// Returns the path to the folder where Adom logs are stored. Does not verify
/// that the directory exists.
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    let mut p = cfg.adom_home.clone();
    p.push("log");
    Ok(p)
}
...
"#;
    ...
    fn config_defaults_to_file_cli_auth_store_mode() -> std::io::Result<()> {
        let adom_home = TempDir::new()?;
        let cfg = ConfigToml::default();

        let config = Config::load_from_base_config_with_overrides(
            cfg,
            ConfigOverrides::default(),
            adom_home.path().to_path_buf(),
        )?;
    ...
    async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
        let adom_home = TempDir::new()?;
        let managed_path = adom_home.path().join("managed_config.toml");
        let config_path = adom_home.path().join(CONFIG_TOML_FILE);
        ...
        let root_value = load_resolved_config(adom_home.path(), Vec::new(), overrides).await?;
        let cfg: ConfigToml = root_value.try_into().map_err(|e| {
            tracing::error!("Failed to deserialize overridden config: {e}");
            std::io::Error::new(std::io::ErrorKind::InvalidData, e)
        })?;
    ...
    async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        ...
        let servers = load_global_mcp_servers(adom_home.path()).await?;
    ...
    async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        let loaded = load_global_mcp_servers(adom_home.path()).await?;
    ...
    async fn managed_config_wins_over_cli_overrides() -> anyhow::Result<()> {
        ...
        std::fs::write(
            adom_home.path().join(CONFIG_TOML_FILE),
            "model = \"base\"\n",
        )?;
...
"#,
        ...
        let err = load_global_mcp_servers(adom_home.path())
            .await
...
"#
    ...
    async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
        ...
        let cwd_path = PathBuf::from("/tmp/adom-mcp");
        let servers = BTreeMap::from([(
            "docs".to_string(),
            McpServerConfig {
                transport: McpServerTransportConfig::Stdio {
        ...
        let serialized = std::fs::read_to_string(&config_path)?;
        assert!(
            serialized.contains(r#"cwd = "/tmp/adom-mcp""#),
        ...
        match &docs.transport {
            McpServerTransportConfig::Stdio { cwd, .. } => {
                assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/adom-mcp")));
            }
            other => panic!("unexpected transport {other:?}"),
        }
...
"#
    ...
    async fn set_model_updates_defaults() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(adom_home.path())
            .set_model(Some("gpt-5-adom"), Some(ReasoningEffort::High))
            .apply()
            .await?;

        let serialized =
            tokio::fs::read_to_string(adom_home.path().join(CONFIG_TOML_FILE)).await?;
        let parsed: ConfigToml = toml::from_str(&serialized)?;

        assert_eq!(parsed.model.as_deref(), Some("gpt-5-adom"));
...
"#,
    ...
    async fn set_model_updates_profile() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(adom_home.path())
            ...
            .set_model(Some("gpt-5-adom"), Some(ReasoningEffort::Medium))
            .apply()
            .await?;
        ...
        assert_eq!(profile.model.as_deref(), Some("gpt-5-adom"));
        assert_eq!(
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn adom_home(&self) -> PathBuf {
            self.adom_home.path().to_path_buf()
        }
    }
    ...
    fn loads_compact_prompt_from_file() -> std::io::Result<()> {
        let adom_home = TempDir::new()?;
        let workspace = adom_home.path().join("workspace");
...
"#;
        ...
        let adom_home_temp_dir = TempDir::new().unwrap();
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                adom_home: fixture.adom_home(),
                history: History::default(),
                file_opener: UriBasedFileOpener::VsCode,
                adom_linux_sandbox_exe: None,
...
projects = { "/Users/mbolin/code/adom4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/adom3" = { trust_level = "trusted" } }
model = "foo""#;
        let mut doc = initial.parse::<DocumentMut>()?;

        // Approve a new directory
        let new_project = Path::new("/Users/mbolin/code/adom2");
...
[projects."/Users/mbolin/code/adom4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/adom3"]
trust_level = "trusted"

[projects."/Users/mbolin/code/adom2"]
trust_level = "trusted"
"#;
        assert_eq!(contents, expected);
