...
pub fn apply_blocking(
    ...
    let config_path = adom_home.join(CONFIG_TOML_FILE);
    ...
    std::fs::create_dir_all(adom_home).with_context(|| {
        format!(
            "failed to create Adom home directory at {}",
            adom_home.display()
        )
    })?;

    let tmp = NamedTempFile::new_in(adom_home)?;
...
pub async fn apply(
    ...
    let adom_home = adom_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&adom_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(adom_home: &Path) -> Self {
        Self {
            adom_home: adom_home.to_path_buf(),
            profile: None,
            edits: Vec::new(),
        }
    }
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.adom_home, self.profile.as_deref(), &self.edits)
    }
...
mod tests {
    ...
    fn blocking_set_model_top_level() {
        ...
        let adom_home = tmp.path();

        apply_blocking(
            adom_home,
            None,
            &[ConfigEdit::SetModel {
                model: Some("gpt-5-adom".to_string()),
        ...
        let contents =
            std::fs::read_to_string(adom_home.join(CONFIG_TOML_FILE)).expect("read config");
        let expected = r#"model = "gpt-5-adom"
...
"#;
    ...
    fn blocking_set_model_preserves_inline_table_contents() {
        ...
        std::fs::write(
            adom_home.join(CONFIG_TOML_FILE),
            r#"profile = "fast"
...
"#,
        ...
        let raw = std::fs::read_to_string(adom_home.join(CONFIG_TOML_FILE)).expect("read config");
...
";
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !adom_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
        ...
        let updated_expected = r#"model = "gpt-5-adom"
model_reasoning_effort = "high"
"#;
        ConfigEditsBuilder::new(adom_home)
            .set_model(Some("gpt-5-adom"), Some(ReasoningEffort::High))
        ...
        contents = std::fs::read_to_string(adom_home.join(CONFIG_TOML_FILE)).expect("read config");
