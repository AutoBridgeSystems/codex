...
use adom_protocol::protocol::FileChange;
use adom_protocol::protocol::HasLegacyEvent;
use adom_protocol::protocol::ItemCompletedEvent;
use adom_protocol::protocol::ItemStartedEvent;
use adom_protocol::protocol::RawResponseItemEvent;
use adom_protocol::protocol::ReviewRequest;
use adom_protocol::protocol::RolloutItem;
use adom_protocol::protocol::SessionSource;
use adom_protocol::protocol::TaskStartedEvent;
use adom_protocol::protocol::TurnAbortReason;
use adom_protocol::protocol::TurnContextItem;
...
use adom_async_utils::OrCancelExt;
use adom_otel::otel_event_manager::OtelEventManager;
use adom_protocol::config_types::ReasoningEffort as ReasoningEffortConfig;
use adom_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
use adom_protocol::models::ContentItem;
use adom_protocol::models::FunctionCallOutputPayload;
use adom_protocol::models::ResponseInputItem;
use adom_protocol::models::ResponseItem;
use adom_protocol::protocol::InitialHistory;
use adom_protocol::user_input::UserInput;
use adom_utils_readiness::Readiness;
use adom_utils_readiness::ReadinessFlag;

pub mod compact;
use self::compact::build_compacted_history;
use self::compact::collect_user_messages;

/// The high-level interface to the Adom system.
...
/// Wrapper returned by [`Adom::spawn`] containing the spawned [`Adom`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct AdomSpawnOk {
    pub adom: Adom,
...
impl Adom {
    /// Spawn a new [`Adom`] and initialize the session.
    pub async fn spawn(
        config: Config,
        auth_manager: Arc<AuthManager>,
        conversation_history: InitialHistory,
        session_source: SessionSource,
    ) -> AdomResult<AdomSpawnOk> {
        ...
        // Generate a unique ID for the lifetime of this Adom session.
    ...
    /// Use sparingly: prefer `submit()` so Adom is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, sub: Submission) -> AdomResult<()> {
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| AdomErr::InternalAgentDied)?;
        Ok(())
    }
...
pub(crate) struct TurnContext {
    ...
    pub(crate) adom_linux_sandbox_exe: Option<PathBuf>,
...
impl Session {
    fn make_turn_context(
        ...
        TurnContext {
            ...
            adom_linux_sandbox_exe: config.adom_linux_sandbox_exe.clone(),
    ...
    async fn new(
        ...
        for (alias, feature) in session_configuration.features.legacy_feature_usages() {
            ...
            let details = if alias == canonical {
                ...
                Some(format!(
                    "Enable it with `--enable {canonical}` or `[features].{canonical}` in config.toml. See https://github.com/openai/adom/blob/main/docs/config.md#feature-flags for details."
                ))
            };
            post_session_configured_events.push(Event {
                id: INITIAL_SUBMIT_ID.to_owned(),
...
/// Operation handlers
mod handlers {
    use crate::adom::Session;
    use crate::adom::SessionSettingsUpdate;
    use crate::adom::TurnContext;

    use crate::adom::spawn_review_thread;
    use crate::config::Config;
    use crate::mcp::auth::compute_auth_statuses;
    ...
    use adom_protocol::custom_prompts::CustomPrompt;
    use adom_protocol::protocol::ErrorEvent;
    use adom_protocol::protocol::Event;
    use adom_protocol::protocol::EventMsg;
    use adom_protocol::protocol::ListCustomPromptsResponseEvent;
    use adom_protocol::protocol::Op;
    use adom_protocol::protocol::ReviewDecision;
    use adom_protocol::protocol::ReviewRequest;
    use adom_protocol::protocol::TurnAbortReason;
    use adom_protocol::user_input::UserInput;
    use std::sync::Arc;
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| adom_protocol::message_history::HistoryEntry {
    ...
    pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
        ...
        info!("Shutting down Adom instance");
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        adom_linux_sandbox_exe: parent_turn_context.adom_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
    };
...
pub(crate) async fn run_task(
    ...
    // Although from the perspective of adom.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        .await
        {
            ...
            Err(AdomErr::TurnAborted {
                ...
                return Err(AdomErr::TurnAborted {
                    dangling_artifacts: processed_items,
                });
            }
            Err(AdomErr::Interrupted) => return Err(AdomErr::Interrupted),
            Err(AdomErr::EnvVar(var)) => return Err(AdomErr::EnvVar(var)),
            Err(e @ AdomErr::Fatal(_)) => return Err(e),
            Err(e @ AdomErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(e);
            }
            Err(AdomErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, rate_limits).await;
                }
                return Err(AdomErr::UsageLimitReached(e));
            }
            Err(AdomErr::UsageNotIncluded) => return Err(AdomErr::UsageNotIncluded),
            Err(e @ AdomErr::QuotaExceeded) => return Err(e),
            Err(e @ AdomErr::RefreshTokenFailed(_)) => return Err(e),
            Err(e) => {
                ...
                if retries < max_retries {
                    retries += 1;
                    let delay = match e {
                        AdomErr::Stream(_, Some(delay)) => delay,
                        _ => backoff(retries),
                    };
                    warn!(
...
async fn try_run_turn(
    ...
    let mut output: FuturesOrdered<BoxFuture<AdomResult<ProcessedResponseItem>>> =
        FuturesOrdered::new();
    ...
    loop {
        ...
        let event = match stream.next().or_cancel(&cancellation_token).await {
            Ok(event) => event,
            Err(adom_async_utils::CancelErr::Cancelled) => {
            ...
            ResponseEvent::OutputItemDone(item) => {
                ...
                match ToolRouter::build_tool_call(sess.as_ref(), item.clone()) {
                    ...
                    Err(FunctionCallError::Fatal(message)) => {
                        return Err(AdomErr::Fatal(message));
                    }
                }
            }
...
fn mcp_init_error_display(
        ...
        && http_headers.as_ref().map(HashMap::is_empty).unwrap_or(true)
    {
        ...
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = ADOM_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
    } else if is_mcp_client_auth_required_error(err) {
        format!(
            "The {server_name} MCP server is not logged in. Run `adom mcp login {server_name}`."
        )
...
mod tests {
    ...
    use adom_protocol::models::ContentItem;
    use adom_protocol::models::ResponseItem;
    use adom_protocol::protocol::McpAuthStatus;
    ...
    pub(crate) fn make_session_and_context() -> (Session, TurnContext) {
        ...
        let adom_home = tempfile::tempdir().expect("create temp dir");
