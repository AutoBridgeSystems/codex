...
//!
//! The history is stored at `~/.adom/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.adom`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.adom_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.adom/history.jsonl` and ensure the parent directory exists.
