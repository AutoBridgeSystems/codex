use crate::adom::ProcessedResponseItem;
...
use chrono::Local;
use chrono::Utc;
use adom_async_utils::CancelErr;
use adom_protocol::ConversationId;
use adom_protocol::protocol::RateLimitSnapshot;
...
pub type Result<T> = std::result::Result<T, AdomErr>;
...
pub enum AdomErr {
    ...
    #[error(
        "Adom ran out of room in the model's context window. Start a new conversation or clear earlier history before retrying."
    )]
    ...
    /// could not be captured). Analogous to the previous `AdomError::Spawn` variant.
    ...
    #[error(
        "To use Adom with your ChatGPT plan, upgrade to Plus: https://openai.com/chatgpt/pricing."
    )]
    UsageNotIncluded,
    ...
    #[error("adom-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for AdomErr {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://openai.com/chatgpt/pricing), visit https://chatgpt.com/adom/settings/usage to purchase more credits{}",
            ...
            Some(PlanType::Known(KnownPlan::Free)) => {
                "You've hit your usage limit. Upgrade to Plus to continue using Adom (https://openai.com/chatgpt/pricing)."
                    .to_string()
            }
            Some(PlanType::Known(KnownPlan::Pro)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/adom/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
impl AdomErr {
    /// Minimal shim so that existing `e.downcast_ref::<AdomErr>()` checks continue to compile
...
pub fn get_error_message_ui(e: &AdomErr) -> String {
    let message = match e {
        AdomErr::Sandbox(SandboxErr::Denied { output }) => {
        ...
        AdomErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
...
mod tests {
    ...
    use chrono::TimeZone;
    use chrono::Utc;
    use adom_protocol::protocol::RateLimitWindow;
    ...
    fn usage_limit_reached_error_formats_plus_plan() {
        ...
        assert_eq!(
            err.to_string(),
            "You've hit your usage limit. Upgrade to Pro (https://openai.com/chatgpt/pricing), visit https://chatgpt.com/adom/settings/usage to purchase more credits or try again later."
        );
    }
    ...
    fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
        ...
        let err = AdomErr::Sandbox(SandboxErr::Denied {
    ...
    fn usage_limit_reached_error_formats_pro_plan_with_reset() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Visit https://chatgpt.com/adom/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
    ...
    fn usage_limit_reached_includes_hours_and_minutes() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Upgrade to Pro (https://openai.com/chatgpt/pricing), visit https://chatgpt.com/adom/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
