...
use crate::flags::ADOM_RS_SSE_FIXTURE;
...
impl ModelClient {
    ...
    async fn stream_responses(&self, prompt: &Prompt) -> Result<ResponseStream> {
        if let Some(path) = &*ADOM_RS_SSE_FIXTURE {
        ...
        match res {
            Ok(resp) if resp.status().is_success() => {
                ...
                let stream = resp.bytes_stream().map_err(move |e| {
                    AdomErr::ResponseStreamFailed(ResponseStreamFailed {
                        source: e,
            ...
            Ok(res) => {
                    ...
                    && let Err(err) = manager.refresh_token().await
                {
                    let stream_error = match err {
                        RefreshTokenError::Permanent(failed) => {
                            StreamAttemptError::Fatal(AdomErr::RefreshTokenFailed(failed))
                        }
                        RefreshTokenError::Transient(other) => {
                            StreamAttemptError::RetryableTransportError(AdomErr::Io(other))
                        }
                    };
                    ...
                    || status.is_server_error())
                {
                    ...
                    return Err(StreamAttemptError::Fatal(AdomErr::UnexpectedStatus(
                ...
                if status == StatusCode::TOO_MANY_REQUESTS {
                    ...
                    if let Some(ErrorResponse { error }) = body {
                        if error.r#type.as_deref() == Some("usage_limit_reached") {
                            ...
                            let plan_type = error
                                .plan_type
                                .or_else(|| auth.as_ref().and_then(AdomAuth::get_plan_type));
                            let resets_at = error
                                .resets_at
                                .and_then(|seconds| DateTime::<Utc>::from_timestamp(seconds, 0));
                            let adom_err = AdomErr::UsageLimitReached(UsageLimitReachedError {
                                plan_type,
                                resets_at,
                                rate_limits: rate_limit_snapshot,
                            });
                            return Err(StreamAttemptError::Fatal(adom_err));
                        } else if error.r#type.as_deref() == Some("usage_not_included") {
                            return Err(StreamAttemptError::Fatal(AdomErr::UsageNotIncluded));
                        } else if is_quota_exceeded_error(&error) {
                            return Err(StreamAttemptError::Fatal(AdomErr::QuotaExceeded));
                        }
                    }
                }
            ...
            Err(e) => Err(StreamAttemptError::RetryableTransportError(
                AdomErr::ConnectionFailed(ConnectionFailedError { source: e }),
            )),
        }
    }
...
enum StreamAttemptError {
    ...
    RetryableTransportError(AdomErr),
    Fatal(AdomErr),
}
...
impl StreamAttemptError {
    ...
    fn into_error(self) -> AdomErr {
        match self {
            Self::RetryableHttpError {
                status, request_id, ..
            } => {
                if status == StatusCode::INTERNAL_SERVER_ERROR {
                    AdomErr::InternalServerError
                } else {
                    AdomErr::RetryLimit(RetryLimitReachedError { status, request_id })
                }
            }
...
fn parse_rate_limit_snapshot(headers: &HeaderMap) -> Option<RateLimitSnapshot> {
    let primary = parse_rate_limit_window(
        headers,
        "x-adom-primary-used-percent",
        "x-adom-primary-window-minutes",
        "x-adom-primary-reset-at",
    );

    let secondary = parse_rate_limit_window(
        headers,
        "x-adom-secondary-used-percent",
        "x-adom-secondary-window-minutes",
        "x-adom-secondary-reset-at",
    );
    ...
    S: Stream<Item = Result<Bytes>> + Unpin,
{
    ...
    let mut response_error: Option<AdomErr> = None;
    ...
    loop {
        ...
        let sse = match response {
            ...
            Ok(Some(Err(e))) => {
                debug!("SSE Error: {e:#}");
                let event = AdomErr::Stream(e.to_string(), None);
            ...
            Ok(None) => {
                match response_completed {
                    ...
                    None => {
                        let error = response_error.unwrap_or(AdomErr::Stream(
        ...
        match event.kind.as_str() {
            ...
            "response.failed" => {
                if let Some(resp_val) = event.response {
                    response_error = Some(AdomErr::Stream(
                        ...
                        match serde_json::from_value::<Error>(error.clone()) {
                            Ok(error) => {
                                if is_context_window_error(&error) {
                                    response_error = Some(AdomErr::ContextWindowExceeded);
                                } else if is_quota_exceeded_error(&error) {
                                    response_error = Some(AdomErr::QuotaExceeded);
                                } else {
                                    let delay = try_parse_retry_after(&error);
                                    let message = error.message.clone().unwrap_or_default();
                                    response_error = Some(AdomErr::Stream(message, delay));
                                }
                            }
                            Err(e) => {
                                let error = format!("failed to parse ErrorResponse: {e}");
                                debug!(error);
                                response_error = Some(AdomErr::Stream(error, None))
                            }
                        }
                    }
                }
...
async fn stream_from_fixture(
    ...
    let stream = ReaderStream::new(rdr).map_err(AdomErr::Io);
    tokio::spawn(process_sse(
        stream,
        tx_event,
...
mod tests {
    ...
    async fn collect_events(
        ...
        let stream = ReaderStream::new(reader).map_err(AdomErr::Io);
    ...
    async fn run_sse(
        ...
        let stream = ReaderStream::new(std::io::Cursor::new(body)).map_err(AdomErr::Io);
        tokio::spawn(process_sse(
            stream,
            tx,
            provider.stream_idle_timeout(),
    ...
    async fn context_window_error_is_fatal() {
        ...
        match &events[0] {
            Err(err @ AdomErr::ContextWindowExceeded) => {
                assert_eq!(err.to_string(), AdomErr::ContextWindowExceeded.to_string());
            }
    ...
    async fn quota_exceeded_error_is_fatal() {
        ...
        match &events[0] {
            Err(err @ AdomErr::QuotaExceeded) => {
                assert_eq!(err.to_string(), AdomErr::QuotaExceeded.to_string());
            }
