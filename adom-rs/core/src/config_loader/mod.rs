...
#[cfg(unix)]
const ADOM_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/adom/managed_config.toml";
...
pub async fn load_config_as_toml(adom_home: &Path) -> io::Result<TomlValue> {
    load_config_as_toml_with_overrides(adom_home, LoaderOverrides::default()).await
}
...
pub(crate) async fn load_config_layers_with_overrides(
    ...
    load_config_layers_internal(adom_home, overrides).await
}
...
async fn load_config_as_toml_with_overrides(
    ...
    let layers = load_config_layers_internal(adom_home, overrides).await?;
...
async fn load_config_layers_internal(
    ...
    let managed_config_path =
        managed_config_path.unwrap_or_else(|| managed_config_default_path(adom_home));

    let user_config_path = adom_home.join(CONFIG_TOML_FILE);
...
fn managed_config_default_path(adom_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = adom_home;
        PathBuf::from(ADOM_MANAGED_CONFIG_SYSTEM_PATH)
    }

    #[cfg(not(unix))]
    {
        adom_home.join("managed_config.toml")
    }
}
