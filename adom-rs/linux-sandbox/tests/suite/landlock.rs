#![cfg(target_os = "linux")]
use adom_core::config::types::ShellEnvironmentPolicy;
use adom_core::error::AdomErr;
use adom_core::error::SandboxErr;
use adom_core::exec::ExecParams;
use adom_core::exec::SandboxType;
use adom_core::exec::process_exec_tool_call;
...
async fn run_cmd(cmd: &[&str], writable_roots: &[PathBuf], timeout_ms: u64) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_adom-linux-sandbox");
    let adom_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_adom-linux-sandbox");
    let adom_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
    ...
    let output = match result {
        Ok(output) => output,
        Err(AdomErr::Sandbox(SandboxErr::Denied { output })) => *output,
        _ => {
