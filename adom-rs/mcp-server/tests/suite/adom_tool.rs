...
use adom_core::parse_command;
use adom_core::protocol::FileChange;
use adom_core::protocol::ReviewDecision;
use adom_core::spawn::ADOM_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use adom_mcp_server::AdomToolCallParam;
use adom_mcp_server::ExecApprovalElicitRequestParams;
use adom_mcp_server::ExecApprovalResponse;
use adom_mcp_server::PatchApprovalElicitRequestParams;
use adom_mcp_server::PatchApprovalResponse;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(ADOM_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Adom sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "adom" tool request, which should hit the completions endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let adom_request_id = mcp_process
        .send_adom_tool_call(AdomToolCallParam {
    ...
    let expected_elicitation_request = create_expected_elicitation_request(
        ...
        adom_request_id.to_string(),
        params.adom_event_id.clone(),
    )?;
    ...
    // Verify the original `adom` tool call completes and that the file was created.
    let adom_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(adom_request_id)),
    )
    .await??;
    assert_eq!(
        JSONRPCResponse {
            jsonrpc: JSONRPC_VERSION.into(),
            id: RequestId::Integer(adom_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request(
    ...
    command: Vec<String>,
    workdir: &Path,
    adom_mcp_tool_call_id: String,
    adom_event_id: String,
) -> anyhow::Result<JSONRPCRequest> {
    let expected_message = format!(
        "Allow Adom to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let adom_parsed_cmd = parse_command::parse_command(&command);
    Ok(JSONRPCRequest {
        jsonrpc: JSONRPC_VERSION.into(),
        ...
        params: Some(serde_json::to_value(&ExecApprovalElicitRequestParams {
            ...
            adom_elicitation: "exec-approval".to_string(),
            adom_mcp_tool_call_id,
            adom_event_id,
            adom_command: command,
            adom_cwd: workdir.to_path_buf(),
            adom_call_id: "call1234".to_string(),
            adom_parsed_cmd,
            adom_risk: None,
        })?),
    })
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "adom" tool request that will trigger the apply_patch command
    let adom_request_id = mcp_process
        .send_adom_tool_call(AdomToolCallParam {
    ...
    // Verify the original `adom` tool call completes
    let adom_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Integer(adom_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_adom_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = adom_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn adom_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `adom mcp` with a specific config.toml.
    let adom_home = TempDir::new()?;
    create_config_toml(adom_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(adom_home.path()).await?;
...
fn create_expected_patch_approval_elicitation_request(
    ...
    message_lines.push("Allow Adom to apply proposed code changes?".to_string());
    ...
    Ok(JSONRPCRequest {
        ...
        params: Some(serde_json::to_value(&PatchApprovalElicitRequestParams {
            ...
            adom_elicitation: "patch-approval".to_string(),
            adom_mcp_tool_call_id,
            adom_event_id,
            adom_reason: reason,
            adom_grant_root: grant_root,
            adom_changes: changes,
            adom_call_id: "call1234".to_string(),
        })?),
    })
}
...
/// Create a Adom config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(adom_home: &Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = adom_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
