...
use crate::adom_tool_config::AdomToolCallParam;
use crate::adom_tool_config::AdomToolCallReplyParam;
use crate::adom_tool_config::create_tool_for_adom_tool_call_param;
use crate::adom_tool_config::create_tool_for_adom_tool_call_reply_param;
...
use adom_core::protocol::Submission;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_adom_uuid: Arc<Mutex<HashMap<RequestId, ConversationId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            running_requests_id_to_adom_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let result = mcp_types::InitializeResult {
            ...
            server_info: mcp_types::Implementation {
                name: "adom-mcp-server".to_string(),
                version: env!("CARGO_PKG_VERSION").to_string(),
                title: Some("Adom".to_string()),
                user_agent: Some(get_adom_user_agent()),
            },
        };
    ...
    async fn handle_list_tools(
        ...
        let result = ListToolsResult {
            tools: vec![
                create_tool_for_adom_tool_call_param(),
                create_tool_for_adom_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(
        ...
        match name.as_str() {
            "adom" => self.handle_tool_call_adom(id, arguments).await,
            "adom-reply" => {
                self.handle_tool_call_adom_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_adom(&self, id: RequestId, arguments: Option<serde_json::Value>) {
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<AdomToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg
                    .into_config(self.adom_linux_sandbox_exe.clone())
                    .await
                {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![ContentBlock::TextContent(TextContent {
                                ...
                                text: format!(
                                    "Failed to load Adom configuration from overrides: {e}"
                                ),
                                annotations: None,
                            })],
                            is_error: Some(true),
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for Adom tool: {e}"),
                            annotations: None,
                        })],
                        is_error: Some(true),
                        structured_content: None,
            ...
            None => {
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_string(),
                        text:
                            "Missing arguments for adom tool-call; the `prompt` field is required."
                                .to_string(),
                        annotations: None,
                    })],
                    is_error: Some(true),
        ...
        let running_requests_id_to_adom_uuid = self.running_requests_id_to_adom_uuid.clone();

        // Spawn an async task to handle the Adom session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Adom session and stream events back to the client.
            crate::adom_tool_runner::run_adom_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                conversation_manager,
                running_requests_id_to_adom_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_adom_session_reply(
        &self,
        ...
        // parse arguments
        let AdomToolCallReplyParam {
            conversation_id,
            prompt,
        } = match arguments {
            Some(json_val) => match serde_json::from_value::<AdomToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Adom tool call reply parameters: {e}");
                    let result = CallToolResult {
                        content: vec![ContentBlock::TextContent(TextContent {
                            r#type: "text".to_owned(),
                            text: format!("Failed to parse configuration for Adom tool: {e}"),
            ...
            None => {
                tracing::error!(
                    "Missing arguments for adom-reply tool-call; the `conversation_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![ContentBlock::TextContent(TextContent {
                        r#type: "text".to_owned(),
                        text: "Missing arguments for adom-reply tool-call; the `conversation_id` and `prompt` fields are required.".to_owned(),
                        annotations: None,
                    })],
                    is_error: Some(true),
                    structured_content: None,
        ...
        tokio::spawn({
            ...
            let running_requests_id_to_adom_uuid = running_requests_id_to_adom_uuid.clone();

            async move {
                crate::adom_tool_runner::run_adom_tool_session_reply(
                    adom,
                    outgoing,
                    request_id,
                    prompt,
    ...
    async fn handle_cancelled_notification(
        ...
        let conversation_id = {
            let map_guard = self.running_requests_id_to_adom_uuid.lock().await;
        ...
        // Obtain the Adom conversation from the server.
        let adom_arc = match self
            .conversation_manager
            .get_conversation(conversation_id)
            .await
        {
        ...
        // Submit interrupt to Adom.
        let err = adom_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: adom_core::protocol::Op::Interrupt,
            })
            .await;
        if let Err(e) = err {
            tracing::error!("Failed to submit interrupt to Adom: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_adom_uuid
            .lock()
            .await
            .remove(&request_id);
    }
