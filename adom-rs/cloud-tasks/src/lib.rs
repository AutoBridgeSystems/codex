...
struct ApplyJob {
    task_id: adom_cloud_tasks_client::TaskId,
    diff_override: Option<String>,
}

struct BackendContext {
    backend: Arc<dyn adom_cloud_tasks_client::CloudBackend>,
    base_url: String,
}

async fn init_backend(user_agent_suffix: &str) -> anyhow::Result<BackendContext> {
    let use_mock = matches!(
        std::env::var("ADOM_CLOUD_TASKS_MODE").ok().as_deref(),
        Some("mock") | Some("MOCK")
    );
    let base_url = std::env::var("ADOM_CLOUD_TASKS_BASE_URL")
    ...
    if use_mock {
        return Ok(BackendContext {
            backend: Arc::new(adom_cloud_tasks_client::MockClient),
            base_url,
        });
    }

    let ua = adom_core::default_client::get_adom_user_agent();
    let mut http = adom_cloud_tasks_client::HttpClient::new(base_url.clone())?.with_user_agent(ua);
    ...
    let auth = match adom_core::config::find_adom_home()
        .ok()
        .map(|home| {
            let store_mode = adom_core::config::Config::load_from_base_config_with_overrides(
                adom_core::config::ConfigToml::default(),
                adom_core::config::ConfigOverrides::default(),
                home.clone(),
            )
            .map(|cfg| cfg.cli_auth_credentials_store_mode)
            .unwrap_or_default();
            adom_login::AuthManager::new(home, false, store_mode)
        })
        .and_then(|am| am.auth())
    {
        Some(auth) => auth,
        None => {
            eprintln!(
                "Not signed in. Please run 'adom login' to sign in with ChatGPT, then re-run 'adom cloud'."
            );
            std::process::exit(1);
        }
    };
...
async fn run_exec_command(args: crate::cli::ExecCommand) -> anyhow::Result<()> {
    ...
    let ctx = init_backend("adom_cloud_tasks_exec").await?;
    let prompt = resolve_query_input(query)?;
    let env_id = resolve_environment_id(&ctx, &environment).await?;
    let created = adom_cloud_tasks_client::CloudBackend::create_task(
        &*ctx.backend,
        &env_id,
        &prompt,
        "main",
...
async fn resolve_environment_id(ctx: &BackendContext, requested: &str) -> anyhow::Result<String> {
    ...
    match label_matches.as_slice() {
        [] => Err(anyhow!(
            "environment '{trimmed}' not found; run `adom cloud` to list available environments"
        )),
        ...
        [first, rest @ ..] => {
            ...
            if rest.iter().all(|row| row.id == *first_id) {
                ...
                Err(anyhow!(
                    "environment label '{trimmed}' is ambiguous; run `adom cloud` to pick the desired environment id"
                ))
            }
        }
    }
...
fn level_from_status(status: adom_cloud_tasks_client::ApplyStatus) -> app::ApplyResultLevel {
    match status {
        adom_cloud_tasks_client::ApplyStatus::Success => app::ApplyResultLevel::Success,
        adom_cloud_tasks_client::ApplyStatus::Partial => app::ApplyResultLevel::Partial,
        adom_cloud_tasks_client::ApplyStatus::Error => app::ApplyResultLevel::Error,
    }
}
...
fn spawn_preflight(
    ...
    tokio::spawn(async move {
        ...
        let result = adom_cloud_tasks_client::CloudBackend::apply_task_preflight(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
fn spawn_apply(
    ...
    tokio::spawn(async move {
        ...
        let result = adom_cloud_tasks_client::CloudBackend::apply_task(
            &*backend,
            task_id.clone(),
            diff_override,
        )
...
/// Entry point for the `adom cloud` subcommand.
pub async fn run_main(cli: Cli, _adom_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let BackendContext { backend, .. } = init_backend("adom_cloud_tasks_tui").await?;
    let backend = backend;
    ...
    let force_internal = matches!(
        std::env::var("ADOM_CLOUD_TASKS_FORCE_INTERNAL")
            .ok()
            .as_deref(),
    ...
    append_error_log(format!(
        ...
        force_internal,
        adom_core::default_client::get_adom_user_agent()
    ));
    ...
    // Fetch environment list in parallel so the header can show friendly names quickly.
    {
        ...
        tokio::spawn(async move {
            let base_url = util::normalize_base_url(
                &std::env::var("ADOM_CLOUD_TASKS_BASE_URL")
                    .unwrap_or_else(|_| "https://chatgpt.com/backend-api".to_string()),
            );
            let headers = util::build_chatgpt_headers().await;
            ...
            Some(()) = redraw_rx.recv() => {
                ...
                if let Some(page) = app.new_task.as_mut() {
                    ...
                    if page.composer.is_in_paste_burst() {
                        let _ = frame_tx.send(Instant::now() + adom_tui::ComposerInput::recommended_flush_delay());
                    }
                }
                            ...
                            if let Some(ov) = app.diff_overlay.as_mut() {
                                ...
                                if let (Some(turn_id), true) = (turn_id.clone(), !sibling_turn_ids.is_empty())
                                    && ov.attempts.len() == 1 {
                                        ...
                                        tokio::spawn(async move {
                                            match adom_cloud_tasks_client::CloudBackend::list_sibling_attempts(
                                                &*backend,
                                                task_id.clone(),
                                                turn_id,
                                            )
                        ...
                        app::AppEvent::ApplyFinished { id, result } => {
                            ...
                            match result {
                                Ok(outcome) => {
                                    ...
                                    if matches!(outcome.status, adom_cloud_tasks_client::ApplyStatus::Success) {
                    ...
                    Some(Ok(Event::Key(key))) if matches!(key.kind, KeyEventKind::Press | KeyEventKind::Repeat) => {
                        ...
                        if is_ctrl_o && app.new_task.is_some() {
                            ...
                            if should_fetch {
                                    let tx = tx.clone();
                                    tokio::spawn(async move {
            let base_url = crate::util::normalize_base_url(&std::env::var("ADOM_CLOUD_TASKS_BASE_URL").unwrap_or_else(|_| "https://chatgpt.com/backend-api".to_string()));
            let headers = crate::util::build_chatgpt_headers().await;
                                        let res = crate::env_detect::list_environments(&base_url, &headers).await;
                                        let _ = tx.send(app::AppEvent::EnvironmentsLoaded(res));
                                    });
                        ...
                        if let Some(page) = app.new_task.as_mut() {
                            ...
                            match key.code {
                                ...
                                _ => {
                                    ...
                                    } else if let adom_tui::ComposerAction::Submitted(text) = page.composer.input(key) {
                                            ...
                                            if let Some(env) = page.env_id.clone() {
                                                ...
                                                tokio::spawn(async move {
                                                    let git_ref = if let Ok(cwd) = std::env::current_dir() {
                                                        if let Some(branch) = adom_core::git_info::default_branch_name(&cwd).await {
                                                            branch
                                                        } else if let Some(branch) = adom_core::git_info::current_branch_name(&cwd).await {
                                                            branch
                                                        } else {
                                                            "main".to_string()
                                                        }
                                                    ...
                                                    let result = adom_cloud_tasks_client::CloudBackend::create_task(&*backend, &env, &text, &git_ref, false, best_of_n).await;
                        ...
                        if app.apply_modal.is_some() {
                            ...
                            match key.code {
                                ...
                                KeyCode::Enter => {
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        ...
                                        let title = task.title.clone();
                                        {
                                            ...
                                            tokio::spawn(async move {
                                                match adom_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, diff_id.clone()).await {
                                                    Ok(Some(diff)) => {
                                                        let _ = tx.send(app::AppEvent::DetailsDiffLoaded { id: diff_id, title: diff_title, diff });
                                                    }
                                                    Ok(None) => {
                                                        match adom_cloud_tasks_client::CloudBackend::get_task_text(&*backend, diff_id.clone()).await {
                                                            Ok(text) => {
                                        ...
                                        // Also fetch conversation text even when diff exists
                                        {
                                            ...
                                            let msg_title = title;
                                            tokio::spawn(async move {
                                                if let Ok(text) = adom_cloud_tasks_client::CloudBackend::get_task_text(&*backend, msg_id.clone()).await {
                                                    let evt = app::AppEvent::DetailsMessagesLoaded {
                                                        id: msg_id,
                                                        title: msg_title,
                                                        messages: text.messages,
                                ...
                                KeyCode::Char('a') => {
                                    ...
                                    if let Some(task) = app.tasks.get(app.selected).cloned() {
                                        match adom_cloud_tasks_client::CloudBackend::get_task_diff(&*backend, task.id.clone()).await {
                                            Ok(Some(diff)) => {
...
#[cfg(test)]
mod tests {
    use adom_tui::ComposerAction;
    use adom_tui::ComposerInput;
    use crossterm::event::KeyCode;
