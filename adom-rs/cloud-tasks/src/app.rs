...
pub struct EnvironmentRow {
    ...
    pub is_pinned: bool,
    pub repo_hints: Option<String>, // e.g., "openai/adom"
}
...
use adom_cloud_tasks_client::CloudBackend;
use adom_cloud_tasks_client::TaskId;
use adom_cloud_tasks_client::TaskSummary;
#[derive(Default)]
pub struct App {
...
pub struct AttemptView {
    ...
    pub status: adom_cloud_tasks_client::AttemptStatus,
...
pub enum AppEvent {
    ...
    DetailsMessagesLoaded {
        ...
        attempt_status: adom_cloud_tasks_client::AttemptStatus,
    },
    DetailsFailed {
        id: TaskId,
        title: String,
    ...
    AttemptsLoaded {
        id: TaskId,
        attempts: Vec<adom_cloud_tasks_client::TurnAttempt>,
    },
    /// Background completion of new task submission
    NewTaskSubmitted(Result<adom_cloud_tasks_client::CreatedTask, String>),
    ...
    ApplyFinished {
        id: TaskId,
        result: std::result::Result<adom_cloud_tasks_client::ApplyOutcome, String>,
    },
}
...
mod tests {
    ...
    impl adom_cloud_tasks_client::CloudBackend for FakeBackend {
        async fn list_tasks(
            &self,
            env: Option<&str>,
        ) -> adom_cloud_tasks_client::Result<Vec<TaskSummary>> {
            ...
            for (i, t) in titles.into_iter().enumerate() {
                out.push(TaskSummary {
                    ...
                    title: t.to_string(),
                    status: adom_cloud_tasks_client::TaskStatus::Ready,
                    updated_at: Utc::now(),
                    environment_id: env.map(str::to_string),
                    environment_label: None,
                    summary: adom_cloud_tasks_client::DiffSummary::default(),
                    is_review: false,
        ...
        async fn get_task_diff(
            &self,
            _id: TaskId,
        ) -> adom_cloud_tasks_client::Result<Option<String>> {
            Err(adom_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
        ...
        async fn get_task_messages(
            &self,
            _id: TaskId,
        ) -> adom_cloud_tasks_client::Result<Vec<String>> {
            Ok(vec![])
        }
        async fn get_task_text(
            &self,
            _id: TaskId,
        ) -> adom_cloud_tasks_client::Result<adom_cloud_tasks_client::TaskText> {
            Ok(adom_cloud_tasks_client::TaskText {
                prompt: Some("Example prompt".to_string()),
                messages: Vec::new(),
                turn_id: Some("fake-turn".to_string()),
                sibling_turn_ids: Vec::new(),
                attempt_placement: Some(0),
                attempt_status: adom_cloud_tasks_client::AttemptStatus::Completed,
            })
        }
        ...
        async fn list_sibling_attempts(
            ...
            _task: TaskId,
            _turn_id: String,
        ) -> adom_cloud_tasks_client::Result<Vec<adom_cloud_tasks_client::TurnAttempt>> {
            Ok(Vec::new())
        }
        ...
        ) -> adom_cloud_tasks_client::Result<adom_cloud_tasks_client::ApplyOutcome> {
            Err(adom_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
        ...
        async fn create_task(
            ...
            _qa_mode: bool,
            _best_of_n: usize,
        ) -> adom_cloud_tasks_client::Result<adom_cloud_tasks_client::CreatedTask> {
            Err(adom_cloud_tasks_client::CloudTaskError::Unimplemented(
                "not used in test",
            ))
        }
