...
pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = adom_core::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("adom_cloud_tasks_tui");
    let ua = adom_core::default_client::get_adom_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("adom-cli")),
    );
    if let Ok(home) = adom_core::config::find_adom_home() {
        ...
        .unwrap_or_default();
        let am = adom_login::AuthManager::new(home, false, store_mode);
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/adom/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/adom") {
        return format!("{root}/adom/tasks/{task_id}");
    }
    if normalized.ends_with("/adom") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/adom/tasks/{task_id}")
}
