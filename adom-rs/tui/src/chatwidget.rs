...
use adom_core::config::types::Notifications;
use adom_core::git_info::current_branch_name;
use adom_core::git_info::local_git_branches;
use adom_core::project_doc::DEFAULT_PROJECT_DOC_FILENAME;
use adom_core::protocol::AgentMessageDeltaEvent;
use adom_core::protocol::AgentMessageEvent;
use adom_core::protocol::AgentReasoningDeltaEvent;
use adom_core::protocol::AgentReasoningEvent;
use adom_core::protocol::AgentReasoningRawContentDeltaEvent;
use adom_core::protocol::AgentReasoningRawContentEvent;
use adom_core::protocol::ApplyPatchApprovalRequestEvent;
use adom_core::protocol::BackgroundEventEvent;
use adom_core::protocol::DeprecationNoticeEvent;
use adom_core::protocol::ErrorEvent;
use adom_core::protocol::Event;
use adom_core::protocol::EventMsg;
use adom_core::protocol::ExecApprovalRequestEvent;
use adom_core::protocol::ExecCommandBeginEvent;
use adom_core::protocol::ExecCommandEndEvent;
use adom_core::protocol::ExitedReviewModeEvent;
use adom_core::protocol::ListCustomPromptsResponseEvent;
use adom_core::protocol::McpListToolsResponseEvent;
use adom_core::protocol::McpToolCallBeginEvent;
use adom_core::protocol::McpToolCallEndEvent;
use adom_core::protocol::Op;
use adom_core::protocol::PatchApplyBeginEvent;
use adom_core::protocol::RateLimitSnapshot;
use adom_core::protocol::ReviewRequest;
use adom_core::protocol::StreamErrorEvent;
use adom_core::protocol::TaskCompleteEvent;
use adom_core::protocol::TokenUsage;
use adom_core::protocol::TokenUsageInfo;
use adom_core::protocol::TurnAbortReason;
use adom_core::protocol::TurnDiffEvent;
use adom_core::protocol::UndoCompletedEvent;
use adom_core::protocol::UndoStartedEvent;
use adom_core::protocol::UserMessageEvent;
use adom_core::protocol::ViewImageToolCallEvent;
use adom_core::protocol::WarningEvent;
use adom_core::protocol::WebSearchBeginEvent;
use adom_core::protocol::WebSearchEndEvent;
use adom_protocol::ConversationId;
use adom_protocol::parse_command::ParsedCommand;
use adom_protocol::user_input::UserInput;
...
use chrono::Local;
use adom_common::approval_presets::ApprovalPreset;
use adom_common::approval_presets::builtin_approval_presets;
use adom_common::model_presets::ModelPreset;
use adom_common::model_presets::builtin_model_presets;
use adom_core::AuthManager;
use adom_core::ConversationManager;
use adom_core::protocol::AskForApproval;
use adom_core::protocol::SandboxPolicy;
use adom_core::protocol_config_types::ReasoningEffort as ReasoningEffortConfig;
use adom_file_search::FileMatch;
use adom_protocol::plan_tool::UpdatePlanArgs;
...
const NUDGE_MODEL_SLUG: &str = "gpt-5-adom-mini";
...
pub(crate) struct ChatWidgetInit {
    ...
    pub(crate) feedback: adom_feedback::AdomFeedback,
}
...
pub(crate) struct ChatWidget {
    ...
    feedback: adom_feedback::AdomFeedback,
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: adom_core::protocol::SessionConfiguredEvent) {
        self.bottom_pane
        ...
        // Ask adom-core to enumerate custom prompts for this session.
    ...
    fn on_exec_command_output_delta(
        &mut self,
        _ev: adom_core::protocol::ExecCommandOutputDeltaEvent,
    ) {
    ...
    fn on_patch_apply_end(&mut self, event: adom_core::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    fn on_get_history_entry_response(
        &mut self,
        event: adom_core::protocol::GetHistoryEntryResponseEvent,
    ) {
        let adom_core::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: adom_core::protocol::PatchApplyEndEvent,
    ) {
    ...
    pub(crate) fn new(
        ...
        let adom_op_tx = spawn_agent(config.clone(), app_event_tx.clone(), conversation_manager);
    ...
    pub(crate) fn new_from_existing(
        common: ChatWidgetInit,
        conversation: std::sync::Arc<adom_core::AdomConversation>,
        session_configured: adom_core::protocol::SessionConfiguredEvent,
    ) -> Self {
        let ChatWidgetInit {
            config,
            frame_requester,
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Compact => {
                ...
                self.app_event_tx.send(AppEvent::AdomOp(Op::Compact));
            }
            SlashCommand::Review => {
            ...
            SlashCommand::Logout => {
                if let Err(e) = adom_core::auth::logout(
            ...
            SlashCommand::Undo => {
                self.app_event_tx.send(AppEvent::AdomOp(Op::Undo));
            }
            SlashCommand::Diff => {
            ...
            SlashCommand::TestApproval => {
                ...
                use adom_core::protocol::ApplyPatchApprovalRequestEvent;
                use adom_core::protocol::FileChange;

                self.app_event_tx.send(AppEvent::AdomEvent(Event {
                    id: "1".to_string(),
    ...
    pub(crate) fn handle_adom_event(&mut self, event: Event) {
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        match msg {
            ...
            _ => {
                tracing::trace!("handle_adom_event: {:?}", msg);
            }
        }
    ...
    fn on_exited_review_mode(&mut self, review: ExitedReviewModeEvent) {
        ...
        if let Some(output) = review.review_output {
            ...
            if output.findings.is_empty() {
                ...
                let message_text =
                    adom_core::review_format::format_review_findings_block(&output.findings, None);
    ...
    fn open_rate_limit_switch_prompt(&mut self, preset: ModelPreset) {
        ...
        let switch_actions: Vec<SelectionAction> = vec![Box::new(move |tx| {
            tx.send(AppEvent::AdomOp(Op::OverrideTurnContext {
                cwd: None,
    ...
    pub(crate) fn open_model_popup(&mut self) {
        ...
        self.bottom_pane.show_selection_view(SelectionViewParams {
            ...
            subtitle: Some("Switch the model for this and future Adom CLI sessions".to_string()),
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        for choice in choices.iter() {
            ...
            let show_warning =
                preset.model.starts_with("gpt-5-adom") && effort == ReasoningEffortConfig::High;
    ...
    fn apply_model_and_effort(&self, model: String, effort: Option<ReasoningEffortConfig>) {
        self.app_event_tx
            .send(AppEvent::AdomOp(Op::OverrideTurnContext {
                cwd: None,
                approval_policy: None,
                sandbox_policy: None,
    ...
    pub(crate) fn open_approvals_popup(&mut self) {
            ...
            .forced_auto_mode_downgraded_on_windows
        {
            ...
            let mut header = ColumnRenderable::new();
            header.push(line![
                "Adom forced your settings back to Read Only on this Windows machine.".bold()
            ]);
            header.push(line![
                "To re-enable Auto mode, run Adom inside Windows Subsystem for Linux (WSL) or enable Full Access manually.".dim()
                ]);
            Box::new(header)
        } else {
            Box::new(())
        ...
        for preset in presets.into_iter() {
            ...
            let description = if cfg!(target_os = "windows")
                && preset.id == "auto"
                && adom_core::get_platform_sandbox().is_none()
            {
                Some(format!(
            ...
            let actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    if adom_core::get_platform_sandbox().is_none() {
                        vec![Box::new(|tx| {
    ...
    fn windows_world_writable_flagged(&self) -> bool {
        ...
        match adom_windows_sandbox::preflight_audit_everyone_writable(&self.config.cwd, &env_map) {
            Ok(()) => false,
            Err(_) => true,
        }
    }
    ...
    pub(crate) fn open_full_access_confirmation(&mut self, preset: ApprovalPreset) {
        ...
        let info_line = Line::from(vec![
            "When Adom runs with full access, it can edit any file on your computer and run commands with network, without your approval. "
                .into(),
    ...
    pub(crate) fn open_windows_auto_mode_instructions(&mut self) {
        ...
        header.push(line!["Run Adom inside WSL to enable sandboxed commands."]);
        header.push(line![""]);
    ...
    /// Forward an `Op` directly to adom.
    pub(crate) fn submit_op(&self, op: Op) {
        // Record outbound operation for session replay fidelity.
        crate::session_log::log_outbound_op(&op);
        if let Err(e) = self.adom_op_tx.send(op) {
        ...
        items.push(SelectionItem {
            ...
            actions: vec![Box::new(
                move |tx: &AppEventSender| {
                    tx.send(AppEvent::AdomOp(Op::Review {
        ...
        for option in branches {
            ...
            items.push(SelectionItem {
                ...
                actions: vec![Box::new(move |tx3: &AppEventSender| {
                    tx3.send(AppEvent::AdomOp(Op::Review {
                        review_request: ReviewRequest {
                            prompt: format!(
    ...
    pub(crate) async fn show_review_commit_picker(&mut self, cwd: &Path) {
        let commits = adom_core::git_info::recent_commits(cwd, 100).await;
...
pub(crate) fn show_review_commit_picker_with_entries(
    chat: &mut ChatWidget,
    entries: Vec<adom_core::git_info::CommitLogEntry>,
) {
