...
use crate::version::ADOM_CLI_VERSION;
...
use adom_core::config::types::ReasoningSummaryFormat;
...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            line![
                "See ",
                "https://github.com/openai/adom".cyan().underlined(),
        ...
        let content = text![
            line![
                ...
                " ",
                format!("{ADOM_CLI_VERSION} -> {}", self.latest_version).bold(),
            ],
            update_instruction,
            "",
            "See full release notes:",
            "https://github.com/openai/adom/releases/latest"
                .cyan()
                .underlined(),
        ];
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: adom_core::protocol::ReviewDecision,
) -> Box<dyn HistoryCell> {
...
pub(crate) fn new_session_info(
    ...
    SessionInfoCell(if is_first_event {
        ...
        let header = SessionHeaderHistoryCell::new(
            ...
            reasoning_effort,
            config.cwd.clone(),
            crate::version::ADOM_CLI_VERSION,
        );
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for Adom".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/approvals".into(),
                " - choose what Adom can do without approval".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        // Title line rendered inside the box: ">_ OpenAI Adom (vX)"
        let title_spans: Vec<Span<'static>> = vec![
            Span::from(">_ ").dim(),
            Span::from("OpenAI Adom").bold(),
            Span::from(" ").dim(),
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://github.com/openai/adom/blob/main/docs/config.md#mcp_servers\u{7}MCP docs\u{1b}]8;;\u{7}".underlined(),
