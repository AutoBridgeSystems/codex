...
use adom_core::config::set_project_trusted;
use adom_core::git_info::resolve_root_git_project_for_trust;
...
impl WidgetRef for &TrustDirectoryWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        column.push(Line::from(vec![
            "> ".into(),
            "You are running Adom in ".bold(),
            self.cwd.to_string_lossy().to_string().into(),
        ]));
        column.push("");

        let guidance = if self.is_git_repo {
            "Since this folder is version controlled, you may wish to allow Adom to work in this folder without asking for approval."
        } else {
        ...
        if self.is_git_repo {
            options.push((
                "Yes, allow Adom to work in this folder without asking for approval",
            ...
            options.push((
                "Allow Adom to work in this folder without asking for approval",
                TrustDirectorySelection::Trust,
            ));
            options.push((
...
impl TrustDirectoryWidget {
    fn handle_trust(&mut self) {
        ...
        if let Err(e) = set_project_trusted(&self.adom_home, &target) {
