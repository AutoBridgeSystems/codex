...
pub(crate) const WSL_INSTRUCTIONS: &str = r#"Install WSL2 by opening PowerShell as Administrator and running:
    ...
    # Install and run Adom in WSL
    npm install --global @openai/adom
    adom

    # Additional details and instructions for how to install and run Adom in WSL:
    https://developers.openai.com/adom/windows"#;
...
impl WindowsSetupWidget {
    pub fn new(adom_home: PathBuf) -> Self {
        Self {
            adom_home,
            selection: None,
    ...
    fn handle_continue(&mut self) {
        ...
        match ConfigEditsBuilder::new(&self.adom_home)
...
impl WidgetRef for &WindowsSetupWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "> ".into(),
                "To use all Adom features, we recommend running Adom in Windows Subsystem for Linux (WSL2)".bold(),
            ]),
            Line::from(vec!["  ".into(), "WSL allows Adom to run Agent mode in a sandboxed environment with better data protections in place.".into()]),
            Line::from(vec!["  ".into(), "Learn more: https://developers.openai.com/adom/windows".into()]),
            Line::from(""),
        ];
