...
const BASE_ISSUE_URL: &str = "https://github.com/openai/adom/issues/new?template=2-bug-report.yml";
...
pub(crate) struct FeedbackNoteView {
    ...
    snapshot: adom_feedback::AdomLogSnapshot,
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "adom-logs.log".into()]).into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Adom session logs with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        let snapshot = adom_feedback::AdomFeedback::new().snapshot(None);
