...
// The standalone `adom-tui` binary prints a short help message before the
...
use adom_ollama::DEFAULT_OSS_MODEL;
...
pub async fn run_main(
    ...
    let overrides_cli = adom_common::CliConfigOverrides { raw_overrides };
    ...
    let log_dir = adom_core::config::log_dir(&config)?;
    ...
    // Adom CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("adom-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for adom crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("adom_core=info,adom_tui=info,adom_rmcp_client=info")
        })
    };
    ...
    let feedback = adom_feedback::AdomFeedback::new();
    ...
    if cli.oss {
        adom_ollama::ensure_oss_ready(&config)
            .await
            .map_err(|e| std::io::Error::other(format!("OSS setup failed: {e}")))?;
    }

    let otel = adom_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"));
    ...
    if let Some(provider) = otel.as_ref() {
        let otel_layer = OpenTelemetryTracingBridge::new(&provider.logger).with_filter(
            tracing_subscriber::filter::filter_fn(adom_core::otel_init::adom_export_filter),
        );
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: adom_core::protocol::TokenUsage::default(),
    ...
    let auth_manager = AuthManager::shared(
        initial_config.adom_home.clone(),
        false,
    ...
    let resume_selection = if let Some(id_str) = cli.resume_session_id.as_deref() {
        match find_conversation_path_by_id_str(&config.adom_home, id_str).await? {
            ...
            None => {
                ...
                if let Err(err) = writeln!(
                    std::io::stdout(),
                    "No saved session found with ID {id_str}. Run `adom resume` without an ID to choose from existing sessions."
                ) {
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let adom_home = config.adom_home.clone();
        match AdomAuth::from_auth_storage(&adom_home, config.cli_auth_credentials_store_mode) {
...
mod tests {
    ...
    use adom_core::config::ProjectConfig;
    use adom_core::set_windows_sandbox_enabled;
