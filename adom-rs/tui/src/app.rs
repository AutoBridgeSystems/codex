...
use adom_ansi_escape::ansi_escape_line;
...
impl App {
    ...
    pub async fn run(
        ...
        {
            let should_check = adom_core::get_platform_sandbox().is_some()
                && matches!(
                    app.config.sandbox_policy,
                    adom_core::protocol::SandboxPolicy::WorkspaceWrite { .. }
                )
                && !app
                    .config
                    .notices
    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<bool> {
        match event {
            ...
            AppEvent::AdomEvent(event) => {
                self.chat_widget.handle_adom_event(event);
            }
            ...
            AppEvent::AdomOp(op) => self.chat_widget.submit_op(op),
            ...
            AppEvent::PersistFullAccessWarningAcknowledged => {
                if let Err(err) = ConfigEditsBuilder::new(&self.config.adom_home)
    ...
    pub(crate) fn token_usage(&self) -> adom_core::protocol::TokenUsage {
    ...
    fn spawn_world_writable_scan(
        ...
        tokio::task::spawn_blocking(move || {
            if adom_windows_sandbox::preflight_audit_everyone_writable(&cwd, &env_map).is_err() {
                if apply_preset_on_continue {
                    if let Some(preset) = adom_common::approval_presets::builtin_approval_presets()
                        .into_iter()
                        .find(|p| p.id == "auto")
                    {
...
mod tests {
    ...
    fn make_test_app() -> App {
        ...
        let server = Arc::new(ConversationManager::with_auth(AdomAuth::from_api_key(
            "Test API Key",
        )));
        let auth_manager =
            AuthManager::from_auth_for_testing(AdomAuth::from_api_key("Test API Key"));
        let file_search = FileSearchManager::new(config.cwd.clone(), app_event_tx.clone());
