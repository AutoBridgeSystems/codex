...
use adom_common::model_presets::ReasoningEffortPreset;
...
use adom_core::config::OPENAI_DEFAULT_MODEL;
...
use adom_core::protocol::PatchApplyEndEvent;
use adom_core::protocol::RateLimitWindow;
use adom_core::protocol::ReviewCodeLocation;
use adom_core::protocol::ReviewFinding;
use adom_core::protocol::ReviewLineRange;
use adom_core::protocol::ReviewOutputEvent;
use adom_core::protocol::ReviewRequest;
use adom_core::protocol::StreamErrorEvent;
use adom_core::protocol::TaskCompleteEvent;
use adom_core::protocol::TaskStartedEvent;
...
use adom_protocol::plan_tool::PlanItemArg;
use adom_protocol::plan_tool::StepStatus;
...
fn resumed_initial_messages_render_history() {
    ...
    let configured = adom_core::protocol::SessionConfiguredEvent {
...
async fn helpers_are_available_and_do_not_panic() {
    ...
    let cfg = test_config();
    let conversation_manager = Arc::new(ConversationManager::with_auth(AdomAuth::from_api_key(
        "test",
    )));
    let auth_manager = AuthManager::from_auth_for_testing(AdomAuth::from_api_key("test"));
    let init = ChatWidgetInit {
        ...
        auth_manager,
        feedback: adom_feedback::AdomFeedback::new(),
    };
...
fn make_chatwidget_manual() -> (
    ...
    let bottom = BottomPane::new(BottomPaneParams {
        ...
        enhanced_keys_supported: false,
        placeholder_text: "Ask Adom to do anything".to_string(),
...
fn rate_limit_switch_prompt_skips_when_on_lower_cost_model() {
    ...
    chat.auth_manager =
        AuthManager::from_auth_for_testing(AdomAuth::create_dummy_chatgpt_auth_for_testing());
...
fn rate_limit_switch_prompt_shows_once_per_session() {
    let auth = AdomAuth::create_dummy_chatgpt_auth_for_testing();
    let (mut chat, _, _) = make_chatwidget_manual();
...
fn begin_exec(chat: &mut ChatWidget, call_id: &str, raw_cmd: &str) {
    ...
    let parsed_cmd: Vec<ParsedCommand> = adom_core::parse_command::parse_command(&command);
    chat.handle_adom_event(Event {
...
fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no Adom op to be sent, got {other:?}"),
    }
...
fn slash_undo_sends_op() {
    ...
    match rx.try_recv() {
        Ok(AppEvent::AdomOp(Op::Undo)) => {}
        other => panic!("expected AppEvent::AdomOp(Op::Undo), got {other:?}"),
    }
}
...
fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual();
    let rollout_path = PathBuf::from("/tmp/adom-test-rollout.jsonl");
...
fn review_commit_picker_shows_subjects_without_timestamps() {
    ...
    let entries = vec![
        adom_core::git_info::CommitLogEntry {
...
fn custom_prompt_submit_sends_review_op() {
    ...
    // Expect AppEvent::AdomOp(Op::Review { .. }) with trimmed prompt
    let evt = rx.try_recv().expect("expected one app event");
    match evt {
        AppEvent::AdomOp(Op::Review { review_request }) => {
            assert_eq!(
...
fn custom_prompt_enter_empty_does_not_send() {
    ...
    // No AppEvent::AdomOp should be sent
...
fn interrupt_exec_marks_failed_snapshot() {
    ...
    chat.handle_adom_event(Event {
        id: "call-int".into(),
        msg: EventMsg::TurnAborted(adom_core::protocol::TurnAbortedEvent {
...
fn model_selection_popup_snapshot() {
    ...
    chat.config.model = "gpt-5-adom".to_string();
...
fn approvals_popup_includes_wsl_note_for_auto_mode() {
    ...
    assert_eq!(
        popup.contains("Adom forced your settings back to Read Only on this Windows machine."),
...
fn model_reasoning_selection_popup_snapshot() {
    ...
    let preset = builtin_model_presets(None)
        .into_iter()
        .find(|preset| preset.model == "gpt-5-adom")
        .expect("gpt-5-adom preset");
...
async fn binary_size_transcript_snapshot() {
    // the snapshot in this test depends on gpt-5-adom. Skip for now. We will consider
    // creating snapshots for other models in the future.
    if OPENAI_DEFAULT_MODEL != "gpt-5-adom" {
        return;
    }
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual();
                ...
                if let Some(payload) = v.get("payload") {
                    ...
                    let ev = match ev {
                        Event {
                            ...
                            let parsed_cmd = adom_core::parse_command::parse_command(&e.command);
                            Event {
                                id: ev.id,
                                msg: EventMsg::ExecCommandBegin(ExecCommandBeginEvent {
...
//
// Synthesizes a Adom ExecApprovalRequest event to trigger the approval modal
...
fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    // Expect a AdomOp with PatchApproval carrying the submission id, not call id
    let mut found = false;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::AdomOp(Op::PatchApproval { id, decision }) = app_ev {
            assert_eq!(id, "sub-123");
            assert_matches!(decision, adom_core::protocol::ReviewDecision::Approved);
            found = true;
            break;
        }
    }
...
fn apply_patch_full_flow_integration_like() {
    ...
    // 2) User approves via 'y' and App receives a AdomOp
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('y'), KeyModifiers::NONE));
    let mut maybe_op: Option<Op> = None;
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::AdomOp(op) = app_ev {
            maybe_op = Some(op);
            break;
        }
    }
    let op = maybe_op.expect("expected AdomOp after key press");

    // 3) App forwards to widget.submit_op, which pushes onto adom_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to adom channel");
    match forwarded {
