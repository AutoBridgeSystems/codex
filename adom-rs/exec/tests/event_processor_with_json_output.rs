...
use adom_core::protocol::McpInvocation;
...
use adom_core::protocol::SessionConfiguredEvent;
use adom_core::protocol::WarningEvent;
use adom_core::protocol::WebSearchEndEvent;
use adom_exec::event_processor_with_jsonl_output::EventProcessorWithJsonOutput;
use adom_exec::exec_events::AgentMessageItem;
use adom_exec::exec_events::CommandExecutionItem;
use adom_exec::exec_events::CommandExecutionStatus;
use adom_exec::exec_events::ErrorItem;
use adom_exec::exec_events::ItemCompletedEvent;
use adom_exec::exec_events::ItemStartedEvent;
use adom_exec::exec_events::ItemUpdatedEvent;
use adom_exec::exec_events::McpToolCallItem;
use adom_exec::exec_events::McpToolCallItemError;
use adom_exec::exec_events::McpToolCallItemResult;
use adom_exec::exec_events::McpToolCallStatus;
use adom_exec::exec_events::PatchApplyStatus;
use adom_exec::exec_events::PatchChangeKind;
use adom_exec::exec_events::ReasoningItem;
use adom_exec::exec_events::ThreadErrorEvent;
use adom_exec::exec_events::ThreadEvent;
use adom_exec::exec_events::ThreadItem;
use adom_exec::exec_events::ThreadItemDetails;
use adom_exec::exec_events::ThreadStartedEvent;
use adom_exec::exec_events::TodoItem as ExecTodoItem;
use adom_exec::exec_events::TodoListItem as ExecTodoListItem;
use adom_exec::exec_events::TurnCompletedEvent;
use adom_exec::exec_events::TurnFailedEvent;
use adom_exec::exec_events::TurnStartedEvent;
use adom_exec::exec_events::Usage;
use adom_exec::exec_events::WebSearchItem;
...
fn session_configured_produces_thread_started_event() {
    ...
    let session_id =
        adom_protocol::ConversationId::from_string("67e55044-10b1-426f-9247-bb680e5fe0c8")
            .unwrap();
    ...
    let ev = event(
        ...
        EventMsg::SessionConfigured(SessionConfiguredEvent {
            session_id,
            model: "adom-mini-latest".to_string(),
...
fn task_started_produces_turn_started_event() {
    ...
    let out = ep.collect_thread_events(&event(
        "t1",
        EventMsg::TaskStarted(adom_core::protocol::TaskStartedEvent {
...
fn plan_update_emits_todo_list_started_updated_and_completed() {
    ...
    let complete = event(
        "p3",
        EventMsg::TaskComplete(adom_core::protocol::TaskCompleteEvent {
...
fn error_event_produces_error() {
    ...
    let out = ep.collect_thread_events(&event(
        "e1",
        EventMsg::Error(adom_core::protocol::ErrorEvent {
...
fn stream_error_event_produces_error() {
    ...
    let out = ep.collect_thread_events(&event(
        "e1",
        EventMsg::StreamError(adom_core::protocol::StreamErrorEvent {
...
fn task_complete_produces_turn_completed_with_usage() {
    ...
    let usage = adom_core::protocol::TokenUsage {
        input_tokens: 1200,
    ...
    let info = adom_core::protocol::TokenUsageInfo {
    ...
    let token_count_event = event(
        "e1",
        EventMsg::TokenCount(adom_core::protocol::TokenCountEvent {
            info: Some(info),
            rate_limits: None,
        }),
    );
