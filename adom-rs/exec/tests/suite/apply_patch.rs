...
/// While we may add an `apply-patch` subcommand to the `adom` CLI multitool
/// at some point, we must ensure that the smaller `adom-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::cargo_bin("adom-exec")
        .context("should find binary for adom-exec")?
        .arg(ADOM_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
