...
use adom_common::format_env_display::format_env_display;
use adom_core::config::Config;
use adom_core::config::ConfigOverrides;
use adom_core::config::edit::ConfigEditsBuilder;
use adom_core::config::find_adom_home;
use adom_core::config::load_global_mcp_servers;
use adom_core::config::types::McpServerConfig;
use adom_core::config::types::McpServerTransportConfig;
use adom_core::features::Feature;
use adom_core::mcp::auth::compute_auth_statuses;
use adom_core::protocol::McpAuthStatus;
use adom_rmcp_client::delete_oauth_tokens;
use adom_rmcp_client::perform_oauth_login;
use adom_rmcp_client::supports_oauth_login;

/// [experimental] Launch Adom as an MCP server or manage configured MCP servers.
///
/// Subcommands:
/// - `serve`  — run the MCP server on stdio
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.adom/config.toml`
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    let adom_home = find_adom_home().context("failed to resolve ADOM_HOME")?;
    let mut servers = load_global_mcp_servers(&adom_home)
        .await
        .with_context(|| format!("failed to load MCP servers from {}", adom_home.display()))?;
    ...
    ConfigEditsBuilder::new(&adom_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", adom_home.display()))?;
    ...
    } = transport
    {
        match supports_oauth_login(&url).await {
                ...
                if !config.features.enabled(Feature::RmcpClient) {
                    println!(
                        "MCP server supports login. Add `experimental_use_rmcp_client = true` \
                         to your config.toml and run `adom mcp login {name}` to login."
                    );
                } else {
            ...
            Ok(false) => {}
            Err(_) => println!(
                "MCP server may or may not require login. Run `adom mcp login {name}` to login."
            ),
        }
    }
...
async fn run_login(config_overrides: &CliConfigOverrides, login_args: LoginArgs) -> Result<()> {
    ...
    if !config.features.enabled(Feature::RmcpClient) {
        bail!(
            "OAuth login is only supported when [features].rmcp_client is true in config.toml. See https://github.com/openai/adom/blob/main/docs/config.md#feature-flags for details."
        );
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `adom mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: adom mcp remove {}", get_args.name);
