...
fn adom_command(adom_home: &Path) -> Result<assert_cmd::Command> {
    let mut cmd = assert_cmd::Command::cargo_bin("adom")?;
...
fn list_shows_empty_state() -> Result<()> {
    ...
    let mut cmd = adom_command(adom_home.path())?;
...
async fn list_and_get_render_expected_output() -> Result<()> {
    ...
    let mut add = adom_command(adom_home.path())?;
    add.args([
        "mcp",
        "add",
        "docs",
    ...
    let mut servers = load_global_mcp_servers(adom_home.path()).await?;
    ...
    let mut list_cmd = adom_command(adom_home.path())?;
    ...
    let mut list_json_cmd = adom_command(adom_home.path())?;
    ...
    let mut get_cmd = adom_command(adom_home.path())?;
    ...
    assert!(stdout.contains("remove: adom mcp remove docs"));

    let mut get_json_cmd = adom_command(adom_home.path())?;
    get_json_cmd
