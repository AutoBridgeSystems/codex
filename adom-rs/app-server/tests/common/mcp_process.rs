...
use adom_app_server_protocol::CancelLoginChatGptParams;
use adom_app_server_protocol::ClientInfo;
use adom_app_server_protocol::ClientNotification;
...
impl McpProcess {
    pub async fn new(adom_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(adom_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let std_cmd = StdCommand::cargo_bin("adom-app-server")
            .context("should find binary for adom-mcp-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("adom-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        let params = Some(serde_json::to_value(InitializeParams {
            client_info: ClientInfo {
                name: "adom-app-server-tests".to_string(),
                title: None,
    ...
    fn enqueue_user_message(&mut self, notification: JSONRPCNotification) {
        if notification.method == "adom/event/user_message" {
