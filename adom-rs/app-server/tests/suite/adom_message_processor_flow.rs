...
use adom_app_server_protocol::InputItem;
...
use adom_app_server_protocol::ServerRequest;
...
use adom_core::protocol_config_types::ReasoningSummary;
...
async fn test_adom_jsonrpc_conversation_flow() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    // Temporary Adom home with config pointing at the mock server.
    let adom_home = tmp.path().join("adom_home");
    std::fs::create_dir(&adom_home)?;
    ...
    create_config_toml(&adom_home, &server.uri())?;

    // Start MCP server and initialize.
    let mut mcp = McpProcess::new(&adom_home).await?;
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![adom_app_server_protocol::InputItem::Text {
                text: "text".to_string(),
            }],
        })
        .await?;
    ...
    let task_finished_notification: JSONRPCNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("adom/event/task_complete"),
    )
    .await??;
...
async fn test_send_user_turn_changes_approval_policy_behavior() -> Result<()> {
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": adom_core::protocol::ReviewDecision::Approved }),
    )
    .await?;
...
async fn test_send_user_turn_updates_sandbox_and_cwd_between_turns() -> Result<()> {
    ...
    let exec_begin_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("adom/event/exec_command_begin"),
    )
    .await??;
