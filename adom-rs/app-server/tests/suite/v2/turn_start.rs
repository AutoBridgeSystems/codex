...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Adom turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
    ...
    create_config_toml(adom_home.path(), &server.uri(), "never")?;

    let mut mcp = McpProcess::new(adom_home.path()).await?;
    ...
    assert_eq!(
        started.turn.status,
        adom_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    // Two Adom turns hit the mock model (session start + turn/start).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ];
    ...
    let image_path = adom_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let adom_home = tmp.path().to_path_buf();
    ...
    create_config_toml(adom_home.as_path(), &server.uri(), "untrusted")?;

    let mut mcp = McpProcess::new(adom_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(adom_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(adom_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let server = create_mock_chat_completions_server(responses).await;
    create_config_toml(&adom_home, &server.uri(), "untrusted")?;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(adom_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(adom_app_server_protocol::SandboxPolicy::WorkspaceWrite {
