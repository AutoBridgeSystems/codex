...
use adom_app_server_protocol::NewConversationParams; // reused for overrides shape
...
async fn test_list_and_resume_conversations() -> Result<()> {
    // Prepare a temporary ADOM_HOME with a few fake rollout files.
    ...
    // Expect a adom/event notification with msg.type == sessionConfigured
