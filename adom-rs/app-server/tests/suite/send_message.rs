...
async fn test_send_message_success() -> Result<()> {
    // Spin up a mock completions server that immediately ends the Adom turn.
    // Two Adom turns hit the mock model (session start + send-user-message). Provide two SSE responses.
    let responses = vec![
    ...
    // Create a temporary Adom home with config pointing at the mock server.
...
async fn send_message(
    ...
    let raw_attempt = tokio::time::timeout(
        ...
        mcp.read_stream_until_notification_message("adom/event/raw_response_item"),
    )
    .await;
    assert!(
        raw_attempt.is_err(),
...
async fn test_send_message_session_not_found() -> Result<()> {
    // Start MCP without creating a Adom session
    let adom_home = TempDir::new()?;
    let mut mcp = McpProcess::new(adom_home.path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
...
async fn read_raw_response_item(
    ...
    .await
    .expect("adom/event/raw_response_item notification timeout")
    .expect("adom/event/raw_response_item notification resp");

    let serde_json::Value::Object(params) = raw_notification
        .params
        .expect("adom/event/raw_response_item should have params")
    else {
        panic!("adom/event/raw_response_item should have params");
    };
