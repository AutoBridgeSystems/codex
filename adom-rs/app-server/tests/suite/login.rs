...
use adom_login::login_with_api_key;
...
fn create_config_toml(adom_home: &Path) -> std::io::Result<()> {
...
async fn logout_chatgpt_removes_auth() -> Result<()> {
    ...
    create_config_toml(adom_home.path())?;
    login_with_api_key(
        adom_home.path(),
        "sk-test-key",
        AuthCredentialsStoreMode::File,
    )?;
    assert!(adom_home.path().join("auth.json").exists());

    let mut mcp = McpProcess::new_with_env(adom_home.path(), &[("OPENAI_API_KEY", None)]).await?;
...
async fn login_and_cancel_chatgpt() -> Result<()> {
    ...
    let maybe_note = timeout(
        ...
        mcp.read_stream_until_notification_message("adom/event/login_chat_gpt_complete"),
    )
    .await;
...
fn create_config_toml_forced_login(adom_home: &Path, forced_method: &str) -> std::io::Result<()> {
    let config_toml = adom_home.join("config.toml");
    let contents = format!(
        r#"
model = "mock-model"
...
async fn login_chatgpt_rejected_when_forced_api() -> Result<()> {
    let adom_home = TempDir::new()?;
    create_config_toml_forced_login(adom_home.path(), "api")?;
...
async fn login_chatgpt_includes_forced_workspace_query_param() -> Result<()> {
    let adom_home = TempDir::new()?;
    create_config_toml_forced_workspace(adom_home.path(), "ws-forced")?;
