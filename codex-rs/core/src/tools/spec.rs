...
fn create_index_codebase_tool() -> ToolSpec {
    let mut properties = BTreeMap::new();
    properties.insert(
        "root".to_string(),
        JsonSchema::String {
            description: Some(
                "Directory to index. Accepts an absolute path or a path relative to the session working directory."
                    .to_string(),
            ),
        },
    );
    properties.insert(
        ...
        JsonSchema::String {
            description: Some(
                "Optional Markdown output path. Relative values are resolved inside the root directory."
                    .to_string(),
            ),
        },
    );
    properties.insert(
        ...
        JsonSchema::Array {
            items: Box::new(JsonSchema::String {
                description: Some(
                    "Additional POSIX-style glob pattern to exclude (repeatable).".to_string(),
                ),
            }),
            description: Some(
                "Optional extra exclude globs applied in addition to the default ignore list."
                    .to_string(),
            ),
        },
    );
    properties.insert(
        ...
        JsonSchema::Number {
            description: Some(
                "Optional maximum depth to traverse relative to the root directory. Must be at least 1 when provided."
                    .to_string(),
            ),
        },
    );
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Generates an ls -R style index for a directory and optionally writes it to Markdown on disk."
            .to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
pub(crate) fn build_specs(
    ...
    use crate::tools::handlers::IndexCodebaseHandler;
    ...
    if config
        .experimental_supported_tools
        .contains(&"index_codebase".to_string())
    {
        let index_codebase_handler = Arc::new(IndexCodebaseHandler);
        builder.push_spec_with_parallel_support(create_index_codebase_tool(), false);
        builder.register_handler("index_codebase", index_codebase_handler);
    }
...
mod tests {
    ...
    fn test_parallel_support_flags() {
        ...
        assert!(!find_tool(&tools, "index_codebase").supports_parallel_tool_calls);
    }
    ...
    fn test_test_model_family_includes_sync_tool() {
        ...
        assert!(
            tools
                .iter()
                .any(|tool| tool_name(&tool.spec) == "index_codebase")
        );
    }
