...
pub(crate) const DEFAULT_PROJECT_INDEX_RELATIVE_PATH: &str = "docs/INDEX.md";
        ...
        match cmd {
            ...
            SlashCommand::Index => {
                ...
                if index_target.exists() {
                    let message = format!(
                        "{DEFAULT_PROJECT_INDEX_RELATIVE_PATH} already exists here. Skipping /index to avoid overwriting it."
                    );
                    self.add_info_message(message, None);
                    return;
                }
                const INDEX_PROMPT: &str = include_str!("../prompt_for_index_command.md");
                self.submit_user_message(INDEX_PROMPT.to_string().into());
            }
