...
use crate::chatwidget::DEFAULT_PROJECT_INDEX_RELATIVE_PATH;
...
#[test]
fn slash_index_skips_when_index_exists() {
    let (mut chat, mut rx, mut op_rx) = make_chatwidget_manual();
    let tempdir = tempdir().unwrap();
    let existing_path = tempdir
        .path()
        .join(DEFAULT_PROJECT_INDEX_RELATIVE_PATH);
    if let Some(parent) = existing_path.parent() {
        std::fs::create_dir_all(parent).unwrap();
    }
    std::fs::write(&existing_path, "existing index").unwrap();
    ...
    assert!(
        rendered.contains(DEFAULT_PROJECT_INDEX_RELATIVE_PATH),
        "info message should mention the existing file: {rendered:?}"
    );
    assert!(
        rendered.contains("Skipping /index"),
        "info message should explain why /index was skipped: {rendered:?}"
    );
    assert_eq!(
...
fn slash_index_submits_prompt() {
    ...
    let prompt = include_str!("../../prompt_for_index_command.md");
