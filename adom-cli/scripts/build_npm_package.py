#!/usr/bin/env python3
"""Stage and optionally package the @openai/adom npm module."""
...
ADOM_CLI_ROOT = SCRIPT_DIR.parent
REPO_ROOT = ADOM_CLI_ROOT.parent
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "adom-rs" / "responses-api-proxy" / "npm"
ADOM_SDK_ROOT = REPO_ROOT / "sdk" / "typescript"

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "adom": ["adom", "rg"],
    "adom-responses-api-proxy": ["adom-responses-api-proxy"],
    "adom-sdk": ["adom"],
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the Adom CLI npm package.")
    parser.add_argument(
        "--package",
        choices=("adom", "adom-responses-api-proxy", "adom-sdk"),
        default="adom",
        help="Which npm package to stage (default: adom).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "adom":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/adom.js --version\n"
                    f"    node {staging_dir_str}/bin/adom.js --help\n\n"
                )
            elif package == "adom-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/adom-responses-api-proxy.js --help\n\n"
                )
            else:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="adom-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    if package == "adom":
        ...
        shutil.copy2(ADOM_CLI_ROOT / "bin" / "adom.js", bin_dir / "adom.js")
        rg_manifest = ADOM_CLI_ROOT / "bin" / "rg"
        ...
        package_json_path = ADOM_CLI_ROOT / "package.json"
    elif package == "adom-responses-api-proxy":
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "adom-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "adom-responses-api-proxy.js")
    ...
    elif package == "adom-sdk":
        package_json_path = ADOM_SDK_ROOT / "package.json"
        stage_adom_sdk_sources(staging_dir)
    else:
...
def stage_adom_sdk_sources(staging_dir: Path) -> None:
    package_root = ADOM_SDK_ROOT
    ...
    if not dist_src.exists():
        raise RuntimeError("adom-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="adom-npm-pack-") as pack_dir_str:
