#!/usr/bin/env python3
"""Install Adom native binaries (Rust CLI plus ripgrep helpers)."""
...
DEFAULT_WORKFLOW_URL = "https://github.com/openai/adom/actions/runs/17952349351"  # rust-v0.40.0
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. adom-<target>.zst)
    dest_dir: str  # directory under vendor/<target>/ where the binary is installed
    binary_basename: str  # executable name inside dest_dir (before optional .exe)


BINARY_COMPONENTS = {
    "adom": BinaryComponent(
        artifact_prefix="adom",
        dest_dir="adom",
        binary_basename="adom",
    ),
    "adom-responses-api-proxy": BinaryComponent(
        artifact_prefix="adom-responses-api-proxy",
        dest_dir="adom-responses-api-proxy",
        binary_basename="adom-responses-api-proxy",
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install native Adom binaries.")
    parser.add_argument(
        "--workflow-url",
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " May be repeated. Defaults to 'adom' and 'rg'."
        ),
    )
    parser.add_argument(
        "root",
...
def main() -> int:
    ...
    adom_cli_root = (args.root or ADOM_CLI_ROOT).resolve()
    vendor_dir = adom_cli_root / VENDOR_DIR_NAME
    vendor_dir.mkdir(parents=True, exist_ok=True)

    components = args.components or ["adom", "rg"]
    ...
    with tempfile.TemporaryDirectory(prefix="adom-native-artifacts-") as artifacts_dir_str:
